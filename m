...
Received: from lists.gnu.org (lists.gnu.org [209.51.188.17])
	by mail.lfdr.de (Postfix) with ESMTPS id EB1F079227F
	for <lists+qemu-devel@lfdr.de>; Tue,  5 Sep 2023 14:21:08 +0200 (CEST)
Received: from localhost ([::1] helo=lists1p.gnu.org)
	by lists.gnu.org with esmtp (Exim 4.90_1)
	(envelope-from <qemu-devel-bounces@nongnu.org>)
	id 1qdV1r-0002Pd-Lp; Tue, 05 Sep 2023 08:19:35 -0400
Received: from eggs.gnu.org ([2001:470:142:3::10])
 by lists.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1) (envelope-from <mads@ynddal.dk>) id 1qdV1q-0002PV-FR
 for qemu-devel@nongnu.org; Tue, 05 Sep 2023 08:19:34 -0400
Received: from mr85p00im-hyfv06021301.me.com ([17.58.23.188])
 by eggs.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1) (envelope-from <mads@ynddal.dk>) id 1qdV1n-0000Qb-AH
 for qemu-devel@nongnu.org; Tue, 05 Sep 2023 08:19:34 -0400
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=ynddal.dk; s=sig1;
 t=1693916369; bh=mJ7NQ0GkCAa+UHoG2l8BJC/oaR8G/rJ2PDvuZezjVAw=;
 h=Content-Type:Mime-Version:Subject:From:Date:Message-Id:To;
 b=
Received: from smtpclient.apple (mr38p00im-dlb-asmtp-mailmevip.me.com
 [17.57.152.18])
 by mr85p00im-hyfv06021301.me.com (Postfix) with ESMTPSA id 9D7902150E7B;
 Tue,  5 Sep 2023 12:19:28 +0000 (UTC)
...
Date: Tue, 5 Sep 2023 14:19:15 +0200
Cc: John Snow <jsnow@redhat.com>, Stefan Hajnoczi <stefanha@redhat.com>,
 Cleber Rosa <crosa@redhat.com>
Content-Transfer-Encoding: quoted-printable
Message-Id: <D309A376-7CA6-41C9-93FE-BD88037375E7@ynddal.dk>
References: <20230823085429.20519-1-mads@ynddal.dk>
 <20230823085429.20519-13-mads@ynddal.dk>
To: qemu-devel@nongnu.org
X-Mailer: Apple Mail (2.3731.600.7)
X-Proofpoint-GUID: 1Z9JQTx_P_yYr2z-Y61EwnKjatHSY0ge
X-Proofpoint-ORIG-GUID: 1Z9JQTx_P_yYr2z-Y61EwnKjatHSY0ge
...
X-Proofpoint-Spam-Details: rule=notspam policy=default score=0 suspectscore=0
 ...
 engine=8.12.0-2212070000 definitions=main-2309050108
Received-SPF: pass client-ip=17.58.23.188; envelope-from=mads@ynddal.dk;
...
> index =
0000000000000000000000000000000000000000..1d696a8d5c49d4725c938af8bf25a590=
90192986
> GIT binary patch
> literal 4809
...
> zvOXuQEBK)<Z=3D&*`caHJxuL=3Dj6Ub^|Z3{y=3DFd^=3D#f;SL{8I$%G#DIpkjHj1w?t%`=
&6
...
>             yield (event_mapping[event_name], event_name, =
timestamp_ns, pid) + tuple(args)
>=20
> class Analyzer:
...
> +    A trace file analyzer which processes trace records.
>=20
>     An analyzer can be passed to run() or process().  The begin() =
method is
>     invoked, then each trace record is processed, and finally the =
end() method
> @@ -188,6 +191,11 @@ def _build_fn(self, event):
>             return lambda _, rec: fn(*rec[3:3 + event_argcount])
>=20
>     def _process_event(self, rec_args, *, event, event_id, =
timestamp_ns, pid, **kwargs):
> +        warnings.warn(
> +            "Use of deprecated Analyzer class. Refer to Analyzer2 =
instead.",
> +            DeprecationWarning,
> +        )
> +
...
> +    is invoked. When Analyzer is used as a context-manager (using the =
`with`
> +    statement), begin() and end() are called automatically.
> +
> +    If a method matching a trace event name exists, it is invoked to =
process
> +    that trace record.  Otherwise the catchall() method is invoked.
> +
> +    The methods are called with a set of keyword-arguments. These can =
be ignored
> +    using `**kwargs` or defined like any keyword-argument.
> +
> +    The following keyword-arguments are available, but make sure to =
have an
...
> +
> +    Example:
> +    The following method handles the runstate_set(int new_state) =
trace event::
> +
> +      def runstate_set(self, new_state, **kwargs):
> +          ...
> +
> +    The method can also explicitly take a timestamp keyword-argument =
with the
...
> +
> +    The pid can be included in addition to the timestamp and is =
useful when
> +    dealing with traces from multiple processes:
> +
> +      def runstate_set(self, new_state, *, timestamp_ns, pid, =
**kwargs):
> +          ...
> +    """
> +
...
The .zip is the code I used to benchmark. It was committed by mistake =
and serves no value in the repo.=
