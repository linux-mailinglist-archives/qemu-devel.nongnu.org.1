...
Received: from lists.gnu.org (lists.gnu.org [209.51.188.17])
	by mail.lfdr.de (Postfix) with ESMTPS id A22C9924F93
	for <lists+qemu-devel@lfdr.de>; Wed,  3 Jul 2024 05:30:03 +0200 (CEST)
Received: from localhost ([::1] helo=lists1p.gnu.org)
	by lists.gnu.org with esmtp (Exim 4.90_1)
	(envelope-from <qemu-devel-bounces@nongnu.org>)
	id 1sOqfa-00052J-I9; Tue, 02 Jul 2024 23:28:34 -0400
Received: from eggs.gnu.org ([2001:470:142:3::10])
 by lists.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1) (envelope-from <outgoing@sr.ht>) id 1sOqfX-00051Y-Dt
 for qemu-devel@nongnu.org; Tue, 02 Jul 2024 23:28:31 -0400
Received: from mail-a.sr.ht ([46.23.81.152])
 by eggs.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1) (envelope-from <outgoing@sr.ht>) id 1sOqfD-0000Ru-KK
 for qemu-devel@nongnu.org; Tue, 02 Jul 2024 23:28:24 -0400
DKIM-Signature: a=rsa-sha256; bh=304yZMpdTppkxIaH4Pli6Fdyapdt9qXmOKhQNI8J/8s=; 
 c=simple/simple; d=git.sr.ht; h=From:Date:Subject:Reply-to:To;
 q=dns/txt; s=20240113; t=1719977287; v=1;
 b=
Received: from git.sr.ht (unknown [IPv6:2a03:6000:1813:1337::155])
 by mail-a.sr.ht (Postfix) with ESMTPSA id 626BB2016A
 for <qemu-devel@nongnu.org>; Wed,  3 Jul 2024 03:28:07 +0000 (UTC)
From: ~liuxu <liuxu@git.sr.ht>
Date: Tue, 02 Jul 2024 16:10:44 +0800
Subject: [PATCH qemu] target/riscv: Add Zilsd and Zcmlsd extension support
Message-ID: <171997728731.22972.8946294105229314138-0@git.sr.ht>
X-Mailer: git.sr.ht
...
+
+        tcg_gen_addi_tl(addr, addr, 32);
+        tcg_gen_qemu_st_tl(data, addr, ctx->mem_idx, memop); =20
+
+        return true;
+    }
+
...
+
+    tcg_gen_addi_tl(addr, addr, 32);
+    tcg_gen_qemu_st_tl(data_high, addr, ctx->mem_idx, memop); =20
+    return true;
+}
+
+//zcmlsd c.sdsp
...
+    }
+    if(a->rd=3D=3D0){
+        TCGv dest =3D dest_gpr(ctx, a->rd); =20
+
+        TCGv addr =3D get_address(ctx, a->rs1, a->imm);
...
+
+    TCGv addr =3D get_address(ctx, a->rs1, a->imm);
+    tcg_gen_qemu_st_tl(data_low, addr, ctx->mem_idx, memop);   =20
+
+    tcg_gen_addi_tl(addr, addr, 32);
+    tcg_gen_qemu_st_tl(data_high, addr, ctx->mem_idx, memop); =20
+
