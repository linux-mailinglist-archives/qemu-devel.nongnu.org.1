...
Received: from lists.gnu.org (lists.gnu.org [209.51.188.17])
	by mail.lfdr.de (Postfix) with ESMTPS id 442BDC691AB
	for <lists+qemu-devel@lfdr.de>; Tue, 18 Nov 2025 12:33:28 +0100 (CET)
Received: from localhost ([::1] helo=lists1p.gnu.org)
	by lists.gnu.org with esmtp (Exim 4.90_1)
	(envelope-from <qemu-devel-bounces@nongnu.org>)
	id 1vLJws-0004Dn-JU; Tue, 18 Nov 2025 06:32:38 -0500
Received: from eggs.gnu.org ([2001:470:142:3::10])
 by lists.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1) (envelope-from <alex.bennee@linaro.org>)
 id 1vLJwq-0004Cz-2I
 for qemu-devel@nongnu.org; Tue, 18 Nov 2025 06:32:36 -0500
Received: from mail-wm1-x331.google.com ([2a00:1450:4864:20::331])
 by eggs.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_128_GCM_SHA256:128)
 (Exim 4.90_1) (envelope-from <alex.bennee@linaro.org>)
 id 1vLJwl-0006Jj-Lt
 for qemu-devel@nongnu.org; Tue, 18 Nov 2025 06:32:35 -0500
Received: by mail-wm1-x331.google.com with SMTP id
 5b1f17b1804b1-477632d45c9so39665655e9.2
 for <qemu-devel@nongnu.org>; Tue, 18 Nov 2025 03:32:30 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 d=linaro.org; s=google; t=1763465549; x=1764070349; darn=nongnu.org;
 h=content-transfer-encoding:mime-version:message-id:date:user-agent
 :references:in-reply-to:subject:cc:to:from:from:to:cc:subject:date
 :message-id:reply-to;
 bh=nRNKsPuWfZ28g5e4fKUsruNkhjYsfRzvMV/7yCawOjo=;
 b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 d=1e100.net; s=20230601; t=1763465549; x=1764070349;
 h=content-transfer-encoding:mime-version:message-id:date:user-agent
 :references:in-reply-to:subject:cc:to:from:x-gm-gg
 :x-gm-message-state:from:to:cc:subject:date:message-id:reply-to;
 bh=nRNKsPuWfZ28g5e4fKUsruNkhjYsfRzvMV/7yCawOjo=;
 b=
X-Forwarded-Encrypted: i=1;
 AJvYcCX0ZVsaa3UgaclnX0QtZS3JPWDVaSpyFodXxmrCnV5N5km2zSMtyaNJvdWRmV2/duar/mQKUUxBuf34@nongnu.org
X-Gm-Message-State: AOJu0YwK8/xUvFrCY0dcoIB0ZvM81UO8DuGIPb11wTO0Uar0DdDgRYVN
 eYzb9IiCdqT4n7o4uI77W2ThQ8dqHmI4Iw1fquSnHCS/zCGpcGKSFkYIhlU7zbm4gsw=
X-Gm-Gg: 
 yk
X-Google-Smtp-Source: AGHT+IGbn01bpzidm3vzU/gALnvsftRYR4yF73Ka1Ms9wouHI/z33jSJKDmh/HdWXxAzSPZYtM863g==
X-Received: by 2002:a05:600c:3542:b0:471:115e:87bd with SMTP id
 5b1f17b1804b1-4778fe7d0ecmr149548325e9.26.1763465548954; 
 Tue, 18 Nov 2025 03:32:28 -0800 (PST)
Received: from draig.lan ([185.126.160.19]) by smtp.gmail.com with ESMTPSA id
 5b1f17b1804b1-4779973ddcfsm181140965e9.15.2025.11.18.03.32.27
 (version=TLS1_3 cipher=TLS_AES_256_GCM_SHA384 bits=256/256);
 Tue, 18 Nov 2025 03:32:28 -0800 (PST)
Received: from draig (localhost [IPv6:::1])
 by draig.lan (Postfix) with ESMTP id 39D935F886;
 Tue, 18 Nov 2025 11:32:27 +0000 (GMT)
...
Cc: Paolo Bonzini <pbonzini@redhat.com>,  Fabiano Rosas <farosas@suse.de>,
 ...
 Bouvier <pierrick.bouvier@linaro.org>,  Philippe =?utf-8?Q?Mathieu-Daud?=
 =?utf-8?Q?=C3=A9?=
...
In-Reply-To: <20251112162152.447327-3-tangtao1634@phytium.com.cn> (Tao Tang's
 message of "Thu, 13 Nov 2025 00:21:51 +0800")
References: <20251112162152.447327-1-tangtao1634@phytium.com.cn>
 <20251112162152.447327-3-tangtao1634@phytium.com.cn>
User-Agent: mu4e 1.12.14-pre3; emacs 30.1
Date: Tue, 18 Nov 2025 11:32:27 +0000
Message-ID: <87zf8jk244.fsf@draig.linaro.org>
MIME-Version: 1.0
...
> @@ -60,6 +60,9 @@ libqos_srcs =3D files(
>          'x86_64_pc-machine.c',
...
>=20=20
>  if have_virtfs
> diff --git a/tests/qtest/libqos/qos-smmuv3.c b/tests/qtest/libqos/qos-smm=
uv3.c
...
> + * QOS SMMUv3 Module
> + *
> + * This module provides SMMUv3-specific helper functions for libqos test=
s,
...
> +    ((ste)->word[0] =3D ((ste)->word[0] & ~(0x1 << 0)) |                =
  \
> +                      (((val) & 0x1) << 0))
> +#define QSMMU_STE_SET_CONFIG(ste, val)                                  \
> +    ((ste)->word[0] =3D ((ste)->word[0] & ~(0x7 << 1)) |                =
  \
> +                      (((val) & 0x7) << 1))
> +#define QSMMU_STE_SET_S1FMT(ste, val)                                   \
> +    ((ste)->word[0] =3D ((ste)->word[0] & ~(0x3 << 4)) |                =
  \
> +                      (((val) & 0x3) << 4))
> +
...
> +    ((ste)->word[1] =3D ((ste)->word[1] & ~(0x1f << 27)) |              =
  \
> +                      (((val) & 0x1f) << 27))
> +#define QSMMU_STE_SET_S1STALLD(ste, val)                                \
> +    ((ste)->word[2] =3D ((ste)->word[2] & ~(0x1 << 27)) |               =
  \
> +                      (((val) & 0x1) << 27))
> +#define QSMMU_STE_SET_EATS(ste, val)                                    \
> +    ((ste)->word[2] =3D ((ste)->word[2] & ~(0x3 << 28)) |               =
  \
> +                      (((val) & 0x3) << 28))
> +#define QSMMU_STE_SET_STRW(ste, val)                                    \
> +    ((ste)->word[2] =3D ((ste)->word[2] & ~(0x3 << 30)) |               =
  \
> +                      (((val) & 0x3) << 30))
> +#define QSMMU_STE_SET_NSCFG(ste, val)                                   \
> +    ((ste)->word[2] =3D ((ste)->word[2] & ~(0x3 << 14)) |               =
  \
> +                      (((val) & 0x3) << 14))
...
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x3 << 6)) |                =
  \
> +                      (((val) & 0x3) << 6))
> +#define QSMMU_STE_SET_S2TG(ste, val)                                    \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x3 << 14)) |               =
  \
> +                      (((val) & 0x3) << 14))
> +#define QSMMU_STE_SET_S2PS(ste, val)                                    \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x7 << 16)) |               =
  \
> +                      (((val) & 0x7) << 16))
> +#define QSMMU_STE_SET_S2AA64(ste, val)                                  \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x1 << 19)) |               =
  \
> +                      (((val) & 0x1) << 19))
> +#define QSMMU_STE_SET_S2ENDI(ste, val)                                  \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x1 << 20)) |               =
  \
> +                      (((val) & 0x1) << 20))
> +#define QSMMU_STE_SET_S2AFFD(ste, val)                                  \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x1 << 21)) |               =
  \
> +                      (((val) & 0x1) << 21))
> +#define QSMMU_STE_SET_S2HD(ste, val)                                    \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x1 << 23)) |               =
  \
> +                      (((val) & 0x1) << 23))
> +#define QSMMU_STE_SET_S2HA(ste, val)                                    \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x1 << 24)) |               =
  \
> +                      (((val) & 0x1) << 24))
> +#define QSMMU_STE_SET_S2S(ste, val)                                     \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x1 << 25)) |               =
  \
> +                      (((val) & 0x1) << 25))
> +#define QSMMU_STE_SET_S2R(ste, val)                                     \
> +    ((ste)->word[5] =3D ((ste)->word[5] & ~(0x1 << 26)) |               =
  \
> +                      (((val) & 0x1) << 26))
> +
...
> +     (((val) & 0x1) << 1))
> +#define QSMMU_CD_SET_MAIR0(cd, val) ((cd)->word[6] =3D (val))
> +#define QSMMU_CD_SET_MAIR1(cd, val) ((cd)->word[7] =3D (val))
...
> +    ((cd)->word[2] =3D ((cd)->word[2] & ~(0x1 << 0)) |                  =
  \
> +                     (((val) & 0x1) << 0))
> +#define QSMMU_CD_SET_NSCFG1(cd, val)                                    \
> +    ((cd)->word[4] =3D ((cd)->word[4] & ~(0x1 << 0)) |                  =
  \
> +                     (((val) & 0x1) << 0))
> +
> +
...
> +    if (ctx->tx_space !=3D QSMMU_SPACE_NONSECURE) {
...
> +        /* Non-secure: secure=3D0, space=3D1 */
> +        attrs =3D ITD_ATTRS_SET_SECURE(attrs, 0);
> +        attrs =3D ITD_ATTRS_SET_SPACE(attrs, QSMMU_SPACE_NONSECURE);
> +        break;
> +    default:
...
> +    build_result =3D qsmmu_build_translation(
> +                       ctx->qts, ctx->config.trans_mode,
> +                       ctx->tx_space, ctx->sid);
> +    if (build_result !=3D 0) {
> +        g_test_message("Build failed: mode=3D%u sid=3D%u status=3D0x%x",
> +                       ctx->config.trans_mode, ctx->sid, build_result);
> +        ctx->trans_status =3D build_result;
...
> +    ctx->trans_status =3D qpci_io_readl(ctx->dev, ctx->bar,
...
> +     *
> +     * We only support Non-secure state for now. But in future, this can=
 be
> +     * extended to support static Secure state or dynamic Realm state as=
 well.
> +     */
> +    attrs_val =3D qsmmu_build_dma_attrs(QSMMU_SPACE_NONSECURE);
...
> +    for (i =3D 0; i < 1000; i++) {
> +        result =3D qpci_io_readl(ctx->dev, ctx->bar, ITD_REG_DMA_RESULT);
> +        if (result !=3D ITD_DMA_RESULT_BUSY) {
> +            ctx->dma_result =3D result;
> +            break;
> +        }
> +        g_usleep(1000);
> +    }
...
> +    if (ctx->dma_result =3D=3D ITD_DMA_RESULT_BUSY) {
> +        ctx->dma_result =3D ITD_DMA_ERR_TX_FAIL;
> +    }
> +
...
> +    base_lo =3D qtest_readl(qts, smmu_base + bank_off +
> +                          QSMMU_REG_CMDQ_BASE);
> +    base_hi =3D qtest_readl(qts, smmu_base + bank_off +
> +                          QSMMU_REG_CMDQ_BASE + 4);
> +    base =3D ((uint64_t)base_hi << 32) | base_lo;
> +    log2size =3D base & 0x1f;
> +    qbase =3D base & QSMMU_BASE_ADDR_MASK;
> +
> +    /* Read CMDQ_PROD register */
> +    prod =3D qtest_readl(qts, smmu_base + bank_off +
> +                       QSMMU_REG_CMDQ_PROD);
> +    index_mask =3D (1u << log2size) - 1u;
> +    slot =3D prod & index_mask;
> +    entry_pa =3D qbase + (uint64_t)slot * 16u;
> +
...
> +    new_prod =3D (prod + 1) & ((1u << (log2size + 1)) - 1u);
> +    qtest_writel(qts, smmu_base + bank_off + QSMMU_REG_CMDQ_PROD, new_pr=
od);
> +}
> +
> +void qsmmu_cleanup_translation(QSMMUTestContext *ctx)
> +{
> +    static const QSMMUSpace spaces[] =3D { QSMMU_SPACE_NONSECURE };
> +    uint32_t sid;
...
> +
> +    sid =3D ctx->sid;
> +    ste_addr =3D sid * QSMMU_STE_OR_CD_ENTRY_BYTES + QSMMU_STR_TAB_BASE;
> +
> +    /* Clear page table entries and configuration structures */
> +    for (idx =3D 0; idx < ARRAY_SIZE(spaces); idx++) {
> +        build_space =3D spaces[idx];
> +
> +        ste_addr_real =3D qsmmu_apply_space_offs(build_space, ste_addr);
> +        /* Clear STE (8 words) */
> +        for (i =3D 0; i < 8; i++) {
> +            qtest_writel(ctx->qts, ste_addr_real + i * 4, 0);
> +        }
> +
> +        cd_addr_real =3D qsmmu_apply_space_offs(build_space, QSMMU_CD_GP=
A);
> +        /* Clear CD (8 words) */
> +        for (i =3D 0; i < 8; i++) {
...
> +        qsmmu_push_cfgi_cmd(ctx->qts, ctx->smmu_base, QSMMU_SPACE_NONSEC=
URE,
...
> +{
> +    uint32_t expected =3D qsmmu_expected_dma_result(ctx);
> +    g_test_message("-> Validating result: expected=3D0x%x actual=3D0x%x",
> +                   expected, ctx->dma_result);
> +    return (ctx->dma_result =3D=3D expected);
> +}
> +
...
> +    config_result =3D qsmmu_setup_and_enable_translation(ctx);
> +    if (config_result !=3D 0) {
> +        g_test_message("Configuration failed: mode=3D%u status=3D0x%x",
> +                       ctx->config.trans_mode, config_result);
> +        return;
> +    }
> +
> +    /* Trigger DMA operation */
> +    dma_result =3D qsmmu_trigger_dma(ctx);
> +    if (dma_result !=3D 0) {
> +        g_test_message("DMA failed: mode=3D%u result=3D0x%x",
> +                       ctx->config.trans_mode, dma_result);
> +    } else {
> +        g_test_message("-> DMA succeeded: mode=3D%u", ctx->config.trans_=
mode);
> +    }
> +
> +    /* Validate test result */
> +    test_passed =3D qsmmu_validate_test_result(ctx);
...
> +    for (i =3D 0; i < count; i++) {
> +        QSMMUTestContext ctx =3D {
> +            .qts =3D qts,
> +            .dev =3D dev,
> +            .bar =3D bar,
> +            .smmu_base =3D smmu_base,
> +            .config =3D configs[i],
> +            .trans_status =3D 0,
> +            .dma_result =3D 0,
> +            .sid =3D dev->devfn,
> +            .tx_space =3D qsmmu_sec_sid_to_space(configs[i].sec_sid),
> +        };
> +
> +        qsmmu_single_translation(&ctx);
> +        g_test_message("--> Test %d completed: mode=3D%u sec_sid=3D%u "
> +                       "status=3D0x%x result=3D0x%x", i, configs[i].tran=
s_mode,
> +                       configs[i].sec_sid, ctx.trans_status, ctx.dma_res=
ult);
> +    }
> +}
> +
...
> +    int i;
> +
> +    build_space =3D tx_space;
> +    /* Only Non-Secure space is supported */
> +    if (build_space !=3D QSMMU_SPACE_NONSECURE) {
> +        return 0xdeadbeafu;
> +    }
> +
> +    /* Build STE image */
...
> +
> +    /*
> +     * The consistent policy also extends to pointer fetches. For cases =
that
> +     * require reading STE.S1ContextPtr or STE.S2TTB, we still follow th=
e same
> +     * policy:
...
'.
> +     */
> +    cd_addr_real =3D qsmmu_apply_space_offs(build_space, QSMMU_CD_GPA);
> +    QSMMU_STE_SET_CTXPTR(&ste, cd_addr_real);
> +
> +    vttb =3D QSMMU_VTTB;
> +    vttb_real =3D qsmmu_apply_space_offs(build_space, vttb);
...
> +
> +        for (i =3D 0; i < 8; i++) {
> +            /* TODO: Maybe need more work to write to secure RAM in futu=
re */
> +            qtest_writel(qts, cd_addr_real + i * 4, cd.word[i]);
> +            g_assert_cmpint(qtest_readl(qts, cd_addr_real + i * 4), =3D=
=3D,
> +                            cd.word[i]);
> +        }
> +    }
...
> +    if (mode =3D=3D QSMMU_TM_NESTED) {
> +        /*
> +         * Extra Stage 2 page tables is needed if
> +         *          SMMUTranslationClass =3D=3D SMMU_CLASS_CD
> +         * as smmuv3_do_translate would translate an IPA of the CD to th=
e final
...
> +}
> +
> +void qsmmu_program_bank(QTestState *qts, uint64_t bank_base, QSMMUSpace =
sp)
> +{
> +    uint64_t cmdq_base, eventq_base, strtab_base;
> +
> +    qtest_writel(qts, bank_base + QSMMU_REG_GBPA, 0x80000000);  /* UPDAT=
E */
> +    qtest_writel(qts, bank_base + QSMMU_REG_CR0, 0x0);          /* Disab=
le */
> +    qtest_writel(qts, bank_base + QSMMU_REG_CR1, 0x0d75);       /* Confi=
g */
> +
> +    /* CMDQ_BASE: add address-space offset*/
> +    cmdq_base =3D qsmmu_apply_space_offs(sp, QSMMU_CMDQ_BASE_ADDR);
> +    cmdq_base |=3D 0x0a;  /* Size and valid bits */
...
> +    eventq_base =3D qsmmu_apply_space_offs(sp, QSMMU_EVENTQ_BASE_ADDR);
> +    eventq_base |=3D 0x0a;  /* Size and valid bits */
...
> +
> +    /* STRTAB_BASE_CFG: linear stream table, LOG2SIZE=3D5 */
> +    qtest_writel(qts, bank_base + QSMMU_REG_STRTAB_CFG, 0x5);
> +
> +    /* STRTAB_BASE: add address-space offset */
> +    strtab_base =3D qsmmu_apply_space_offs(sp, QSMMU_STR_TAB_BASE);
...
> +    uint64_t ns_base =3D qsmmu_bank_base(smmu_base, QSMMU_SPACE_NONSECUR=
E);
> +    qsmmu_program_bank(qts, ns_base, QSMMU_SPACE_NONSECURE);
> +
> +    /* Program the requested space if different from Non-Secure */
> +    sp_base =3D qsmmu_bank_base(smmu_base, space);
...
> +}
> +
> +static uint64_t qsmmu_get_table_addr(uint64_t base, int level, uint64_t =
iova)
> +{
> +    uint32_t index =3D qsmmu_get_table_index(iova, level);
...
> + *
> + * IOMMU need to set different attributes for PTEs based on the translat=
ion mode
> + */
> +static uint64_t qsmmu_get_pte_attrs(QSMMUTransMode mode, bool is_leaf,
> +                                    QSMMUSpace space)
> +{
> +    uint64_t rw_mask =3D QSMMU_LEAF_PTE_RW_MASK;
> +    uint64_t ro_mask =3D QSMMU_LEAF_PTE_RO_MASK;
> +    uint64_t non_leaf_mask =3D QSMMU_NON_LEAF_PTE_MASK;
> +
> +    switch (space) {
> +    case QSMMU_SPACE_NONSECURE:
> +        break;
...
> +
> +    /* For leaf PTE */
> +    if (mode =3D=3D QSMMU_TM_NESTED || mode =3D=3D QSMMU_TM_S1_ONLY) {
> +        return rw_mask;
> +    }
> +
> +    return ro_mask;
...
> + *
> + * Calculates and writes a 4-level Stage 2 page table walk for the given=
 IPA.
...
> +    all_s2_l0_pte_val =3D qsmmu_apply_space_offs(
> +        space, QSMMU_L0_PTE_VAL | qsmmu_get_pte_attrs(mode, false, space=
));
> +    all_s2_l1_pte_val =3D qsmmu_apply_space_offs(
> +        space, QSMMU_L1_PTE_VAL | qsmmu_get_pte_attrs(mode, false, space=
));
> +    all_s2_l2_pte_val =3D qsmmu_apply_space_offs(
> +        space, QSMMU_L2_PTE_VAL | qsmmu_get_pte_attrs(mode, false, space=
));
> +
> +    /* Stage 2 Level 0 */
> +    s2_l0_addr =3D qsmmu_get_table_addr(s2_vttb, 0, ipa);
> +    qtest_writeq(qts, s2_l0_addr, all_s2_l0_pte_val);
> +
> +    /* Stage 2 Level 1 */
> +    s2_l1_addr =3D qsmmu_get_table_addr(all_s2_l0_pte_val, 1, ipa);
> +    qtest_writeq(qts, s2_l1_addr, all_s2_l1_pte_val);
> +
> +    /* Stage 2 Level 2 */
> +    s2_l2_addr =3D qsmmu_get_table_addr(all_s2_l1_pte_val, 2, ipa);
> +    qtest_writeq(qts, s2_l2_addr, all_s2_l2_pte_val);
> +
> +    /* Stage 2 Level 3 (leaf) */
> +    s2_l3_addr =3D qsmmu_get_table_addr(all_s2_l2_pte_val, 3, ipa);
> +
> +    /*
> +     * Stage 2 L3 PTE attributes depend on the context:
> +     * - For nested S1 table address translations (!is_final):
> +     *   Use LEAF attrs (0x763) because these PTEs map S1 table pages di=
rectly
> +     * - For final S2 walk (is_final):
> +     *   Use TABLE attrs (0x7e3) for the final IPA=E2=86=92PA mapping
> +     */
> +    if (!is_final) {
> +        all_s2_l3_pte_val =3D
> +            (ipa & QSMMU_PTE_MASK) |
...
> +
> +/*
> + * qsmmu_setup_s1_level_with_nested_s2 - Setup S1 level with nested S2 w=
alk
> + *
> + * @qts: QTest state handle
> + * @space: Security space
...
> + * to translate the S1 table's IPA to PA. This function performs the nes=
ted
...
> + *
> + * The 'SEC_SID' represents the input security state of the device/trans=
action,
> + * whether it's a static Secure state or a dynamically-switched Realm st=
ate.
> + * SEC_SID has been converted to the corresponding SEcurity Space (QSMMU=
Space)
> + * before calling this function.
> + *
> + * In a real SMMU translation, this input security state does not unilat=
erally
> + * determine the output Physical Address (PA) space. The output PA space=
 is
> + * ultimately determined by attributes encountered during the page table=
 walk,
> + * such as NSCFG and NSTable.
> + *
> + * However, for the specific context of testing the SMMU with the iommu-=
testdev,
> + * and to simplify the future support for Secure and Realm states, we ad=
opt a
> + * consistent policy:
> + *
> + * - We always ensure that the page table attributes (e.g., nscfg, nstab=
le)
...
> +
> +    g_test_message("Begin of construction: IOVA=3D0x%lx mode=3D%d is_bui=
lding_CD=3D%s"
> +                   " =3D=3D=3D", iova, mode, is_cd ? "yes" : "no");
> +
...
> +    s1_vttb =3D qsmmu_apply_space_offs(space, QSMMU_VTTB & QSMMU_PTE_MAS=
K);
> +    s2_vttb =3D s1_vttb;
> +
> +    if (!is_cd) {
...
> +
> +        /* Stage 1 Level 0 */
> +        l0_addr =3D qsmmu_get_table_addr(s1_vttb, 0, iova);
> +        qsmmu_setup_s1_level_with_nested_s2(qts, space, 0, l0_addr,
> +                                            all_s2_l0_pte_val, s2_vttb, =
mode);
> +
> +        /* Stage 1 Level 1 */
> +        l1_addr =3D qsmmu_get_table_addr(all_s2_l0_pte_val & QSMMU_PTE_M=
ASK,
> +                                       1, iova);
...
> +
> +        /* Stage 1 Level 2 */
> +        l2_addr =3D qsmmu_get_table_addr(all_s2_l1_pte_val & QSMMU_PTE_M=
ASK,
> +                                       2, iova);
...
> +        l3_addr =3D qsmmu_get_table_addr(all_s2_l2_pte_val & QSMMU_PTE_M=
ASK,
> +                                       3, iova);
> +
> +        s1_leaf_pte_val =3D qsmmu_apply_space_offs(
> +            space, QSMMU_L3_PTE_VAL | qsmmu_get_pte_attrs(mode, true, sp=
ace)
> +        );
> +
> +        qsmmu_setup_s1_level_with_nested_s2(qts, space, 3, l3_addr,
> +                                            s1_leaf_pte_val, s2_vttb, mo=
de);
> +    } else {
> +        /*
> +         * For CD address translation, we start directly with the IPA.
> +         */
> +        s1_leaf_pte_val =3D iova | qsmmu_get_pte_attrs(QSMMU_TM_NESTED,
> +                                                     false, space);
> +    }
> +
> +    /*
> +     * Final Stage 2 walk: Translate the result from Stage 1.
> +     * - For S1_ONLY: This is skipped in hardware but we set it up for t=
esting
> +     * - For S2_ONLY: This is the only walk
> +     * - For NESTED: This translates the IPA from S1 to final PA
> +     * - For CD address (is_cd=3Dtrue): This is a table address, use !is=
_final
> +     */
...
> +    g_test_message("End of construction: PA=3D0x%llx =3D=3D=3D",
> +                   (s1_leaf_pte_val & QSMMU_PTE_MASK) + (iova & 0xfff));
> +}
> diff --git a/tests/qtest/libqos/qos-smmuv3.h b/tests/qtest/libqos/qos-smm=
uv3.h
> new file mode 100644
...
> + *
> + * NOTE: All physical addresses defined here (QSMMU_VTTB, table addresse=
s, etc.)
...
> +    QSMMU_SEC_SID_NONSECURE    =3D 0,
> +    QSMMU_SEC_SID_SECURE       =3D 1,
> +    QSMMU_SEC_SID_REALM        =3D 2,
> +    QSMMU_SEC_SID_ROOT         =3D 3,
> +} QSMMUSecSID;
> +
> +typedef enum QSMMUSpace {
> +    QSMMU_SPACE_SECURE         =3D 0,
> +    QSMMU_SPACE_NONSECURE      =3D 1,
> +    QSMMU_SPACE_ROOT           =3D 2,
> +    QSMMU_SPACE_REALM          =3D 3,
> +} QSMMUSpace;
> +
> +typedef enum QSMMUTransMode {
> +    QSMMU_TM_S1_ONLY           =3D 0,
> +    QSMMU_TM_S2_ONLY           =3D 1,
> +    QSMMU_TM_NESTED            =3D 2,
> +} QSMMUTransMode;
> +
> +typedef struct QSMMUTestConfig {
> +    QSMMUTransMode trans_mode;        /* Translation mode (S1, S2, Neste=
d) */
...
> +    uint32_t expected_result;         /* Expected DMA result for validat=
ion */
> +} QSMMUTestConfig;
> +
...
> +
> +/*
> + * qsmmu_setup_and_enable_translation - Complete translation setup and e=
nable
> + *
> + * @ctx: Test context containing configuration and device handles
> + *
> + * Returns: Translation status (0 =3D success, non-zero =3D error)
> + *
...
> + * @sid: Stream ID
> + *
> + * Returns: Build status (0 =3D success, non-zero =3D error)
> + *
...
> + *
> + * Programs SMMU registers for the requested security space which is cal=
led in
> + * qsmmu_setup_and_enable_translation. Always programs Non-Secure bank f=
irst,
...
> + *
> + * Returns: true if test passed (actual =3D=3D expected), false otherwise
> + *
...
> +
> +/*
> + * qsmmu_setup_translation_tables - Setup complete SMMU page table hiera=
rchy
> + *
> + * @qts: QTest state handle
...
> +
> +void qsmmu_single_translation(QSMMUTestContext *ctx);
> +void qsmmu_translation_batch(const QSMMUTestConfig *configs, size_t coun=
t,
