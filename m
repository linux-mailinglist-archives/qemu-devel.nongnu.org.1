...
Received: from lists.gnu.org (lists.gnu.org [209.51.188.17])
	by mail.lfdr.de (Postfix) with ESMTPS id 8E6F1760B05
	for <lists+qemu-devel@lfdr.de>; Tue, 25 Jul 2023 08:57:10 +0200 (CEST)
Received: from localhost ([::1] helo=lists1p.gnu.org)
	by lists.gnu.org with esmtp (Exim 4.90_1)
	(envelope-from <qemu-devel-bounces@nongnu.org>)
	id 1qOBxq-0008JM-5S; Tue, 25 Jul 2023 02:56:10 -0400
Received: from eggs.gnu.org ([2001:470:142:3::10])
 by lists.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1) (envelope-from <jasowang@redhat.com>)
 id 1qOBxn-0008It-Be
 for qemu-devel@nongnu.org; Tue, 25 Jul 2023 02:56:07 -0400
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
 s=mimecast20190719; t=1690268162;
 h=from:from:reply-to:subject:subject:date:date:message-id:message-id:
 to:to:cc:cc:mime-version:mime-version:content-type:content-type:
 content-transfer-encoding:content-transfer-encoding:
 in-reply-to:in-reply-to:references:references;
 bh=/35RYSXTpJtJMfUAhj2R/tmi4wUT2DEpAoDL/UdnwVA=;
 b=Lyx3SgWQiFMVQzQunXUwbD/YiGuG2cuzouivrX/yTF3CTRpFtXiA7Tx9YxUqDkVyMI6Mda
 P23TT7wSsSNNqY3fNw33E7uoba0lzEpqO/LaVIVsWk4H+Ie5mZGQVsJP80up8q1g8CdJaB
 urCf0D32f2UaeBScYY6ah6skzwueATo=
Received: from mail-lj1-f197.google.com (mail-lj1-f197.google.com
 [209.85.208.197]) by relay.mimecast.com with ESMTP with STARTTLS
 (version=TLSv1.3, cipher=TLS_AES_256_GCM_SHA384) id
 us-mta-257-wV8ytvcgMumaDRch1SkMpA-1; Tue, 25 Jul 2023 02:55:59 -0400
X-MC-Unique: wV8ytvcgMumaDRch1SkMpA-1
Received: by mail-lj1-f197.google.com with SMTP id
 38308e7fff4ca-2b70bfc97e4so47648071fa.2
 for <qemu-devel@nongnu.org>; Mon, 24 Jul 2023 23:55:58 -0700 (PDT)
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 d=1e100.net; s=20221208; t=1690268157; x=1690872957;
 h=content-transfer-encoding:cc:to:subject:message-id:date:from
 :in-reply-to:references:mime-version:x-gm-message-state:from:to:cc
 :subject:date:message-id:reply-to;
 bh=/35RYSXTpJtJMfUAhj2R/tmi4wUT2DEpAoDL/UdnwVA=;
 b=
X-Gm-Message-State: ABy/qLbs2oSWd6J54n6xMlUnsM8kZ1RYCIS2MwcBbmKM5MJHQtA1U90g
 E+QQ3sxKvI26yag6MY5qXIu7LUsSTXj3/5zrEL3sHqIOigzwbIkBjrF+BGimR+n0Dqb5FCVeVB5
 jvquJed6dyS8D202E8mgkYqfyAnigxuHvM8hliDJw9Q==
X-Received: by 2002:a2e:a41c:0:b0:2b7:a64:6c3d with SMTP id
 p28-20020a2ea41c000000b002b70a646c3dmr6917536ljn.44.1690268157044; 
 Mon, 24 Jul 2023 23:55:57 -0700 (PDT)
X-Google-Smtp-Source: APBJJlGtmUMM5lCiFo7qYp47J82pDa9n554xYFcxZN4krZfgG5LjBArl+B6UnDTBLNz89XQHBmy7+3C8mNK/CLL83Ig=
X-Received: by 2002:a2e:a41c:0:b0:2b7:a64:6c3d with SMTP id
 p28-20020a2ea41c000000b002b70a646c3dmr6917522ljn.44.1690268156394; Mon, 24
 Jul 2023 23:55:56 -0700 (PDT)
MIME-Version: 1.0
References: <20230705205914.3457560-1-i.maximets@ovn.org>
 <CACGkMEsO6GaeTmGYphZT7Q3pZ8XhpKon3OWkwFwH9fRWCrgTgQ@mail.gmail.com>
 <95c6b1ac-e262-f1eb-a5dd-9e2769ed5690@ovn.org>
In-Reply-To: <95c6b1ac-e262-f1eb-a5dd-9e2769ed5690@ovn.org>
From: Jason Wang <jasowang@redhat.com>
Date: Tue, 25 Jul 2023 14:55:45 +0800
Message-ID: <CACGkMEtp61rgHej2q63hQbn=5hMkQ=3nE-io5OmoXyqR6-i+wQ@mail.gmail.com>
Subject: Re: [PATCH v2] net: add initial support for AF_XDP network backend
...
On Thu, Jul 20, 2023 at 9:26=E2=80=AFPM Ilya Maximets <i.maximets@ovn.org> =
wrote:
>
> On 7/20/23 09:37, Jason Wang wrote:
> > On Thu, Jul 6, 2023 at 4:58=E2=80=AFAM Ilya Maximets <i.maximets@ovn.or=
g> wrote:
> >>
...
> >> 'xsks-map-fd=3DN' option instead of passing socket file descriptors.
...
> >> 'sock-fds' requires libxdp >=3D 1.4.0.
> >>
...
> >> interfaces.
> >>
> >> However, keeping in mind all of these challenges, current implementati=
on
...
> >>
> >> Inside a VM - iperf3 for basic TCP performance testing and dpdk-testpm=
d
> >> for PPS testing.
> >>
> >> iperf3 result:
...
> >> -        .params     =3D "[user|tap|socket|stream|dgram|vde|bridge|hub=
port|netmap|vhost-user"
> >> +        .params     =3D "[user|tap|socket|stream|dgram|vde|bridge|hub=
port|netmap|af-xdp|vhost-user"
> >>  #ifdef CONFIG_VMNET
...
> >>
> >> +# libxdp
> >> +libxdp =3D dependency('libxdp', required: get_option('af_xdp'), metho=
d: 'pkg-config')
> >> +if libxdp.found() and \
> >> +      not (libbpf.found() and libbpf.version().version_compare('>=3D0=
.7'))
> >> +  libxdp =3D not_found
> >> +  if get_option('af_xdp').enabled()
> >> +    error('af-xdp support requires libbpf version >=3D 0.7')
> >
...
>
> Or do you mean limit the libxdp version to 1.4 in order to avoid conditio=
nal
> on HAVE_XSK_UMEM__CREATE_WITH_FD ?

Yes.

> The problem with that is that libxdp 1.4 is a week+ old, so not available=
 in
...
> >
> >> +  else
> >> +    warning('af-xdp support requires libbpf version >=3D 0.7, disabli=
ng')
> >> +  endif
> >> +endif
> >> +
...
> >> @@ -2112,6 +2124,12 @@ config_host_data.set('CONFIG_HEXAGON_IDEF_PARSE=
R', get_option('hexagon_idef_pars
...
> >> @@ -4285,6 +4303,7 @@ summary_info +=3D {'PVRDMA support':    have_pvr=
dma}
> >>  summary_info +=3D {'fdt support':       fdt_opt =3D=3D 'disabled' ? f=
alse : fdt_opt}
> >>  summary_info +=3D {'libcap-ng support': libcap_ng}
> >>  summary_info +=3D {'bpf support':       libbpf}
> >> +summary_info +=3D {'AF_XDP support':    libxdp}
> >>  summary_info +=3D {'rbd support':       rbd}
> >>  summary_info +=3D {'smartcard support': cacard}
...
> >> +{
> >> +    AFXDPState *s =3D DO_UPCAST(AFXDPState, nc, nc);
> >> +
> >> +    if (s->read_poll !=3D enable || s->write_poll !=3D enable) {
> >> +        s->write_poll =3D enable;
> >> +        s->read_poll  =3D enable;
> >> +        af_xdp_update_fd_handler(s);
> >> +    }
...
> >> +    uint64_t *addr;
> >> +
> >> +    done =3D xsk_ring_cons__peek(&s->cq, XSK_RING_CONS__DEFAULT_NUM_D=
ESCS, &idx);
> >> +
> >> +    for (i =3D 0; i < done; i++) {
> >> +        addr =3D (void *) xsk_ring_cons__comp_addr(&s->cq, idx++);
> >> +        s->pool[s->n_pool++] =3D *addr;
> >> +        s->outstanding_tx--;
> >> +    }
> >> +
> >> +    if (done) {
...
> >> +static void af_xdp_writable(void *opaque)
> >> +{
> >> +    AFXDPState *s =3D opaque;
> >> +
> >> +    /* Try to recover buffers that are already sent. */
> >> +    af_xdp_complete_tx(s);
> >> +
> >> +    /*
> >> +     * Unregister the handler, unless we still have packets to transm=
it
...
> >> +        /*
> >> +         * Out of buffers or space in tx ring.  Poll until we can wri=
te.
...
> >> +    }
> >> +
> >> +    desc =3D xsk_ring_prod__tx_desc(&s->tx, idx);
> >> +    desc->addr =3D s->pool[--s->n_pool];
> >> +    desc->len =3D size;
> >> +
> >> +    data =3D xsk_umem__get_data(s->buffer, desc->addr);
...
> >> +
> >> +    for (i =3D 0; i < n; i++) {
> >> +        *xsk_ring_prod__fill_addr(&s->fq, idx++) =3D s->pool[--s->n_p=
ool];
> >> +    }
> >> +    xsk_ring_prod__submit(&s->fq, n);
> >> +
> >> +    if (xsk_ring_prod__needs_wakeup(&s->fq)) {
> >> +        /* Receive was blocked by not having enough buffers.  Wake it=
 up. */
> >> +        af_xdp_read_poll(s, true);
> >> +    }
> >> +}
...
> >> +static void af_xdp_send(void *opaque)
> >> +{
> >> +    uint32_t i, n_rx, idx =3D 0;
> >> +    AFXDPState *s =3D opaque;
> >> +
> >> +    n_rx =3D xsk_ring_cons__peek(&s->rx, AF_XDP_BATCH_SIZE, &idx);
> >> +    if (!n_rx) {
> >> +        return;
> >> +    }
> >> +
...
> >> +        struct iovec iov;
> >> +
> >> +        desc =3D xsk_ring_cons__rx_desc(&s->rx, idx++);
> >> +
> >> +        iov.iov_base =3D xsk_umem__get_data(s->buffer, desc->addr);
> >> +        iov.iov_len =3D desc->len;
> >> +
> >> +        s->pool[s->n_pool++] =3D desc->addr;
> >> +
> >> +        if (!qemu_sendv_packet_async(&s->nc, &iov, 1,
> >> +                                     af_xdp_send_completed)) {
> >> +            /*
> >> +             * The peer does not receive anymore.  Packet is queued, =
stop
...
> >> +            n_rx =3D xsk_ring_cons__peek(&s->rx, i + 1, &idx);
> >
...
> > __u32 nb, __u32 *idx)
> > {
> >   __u32 entries =3D xsk_cons_nb_avail(cons, nb);
> >
> > if (entries > 0) {
...
> >> +    if (!s->inhibit && nc->queue_index =3D=3D s->n_queues - 1 && s->x=
dp_flags
> >> +        && bpf_xdp_detach(s->ifindex, s->xdp_flags, NULL) !=3D 0) {
> >> +        fprintf(stderr,
> >> +                "af-xdp: unable to remove XDP program from '%s', ifin=
dex: %d\n",
> >> +                s->ifname, s->ifindex);
> >> +    }
> >> +}
> >> +
> >> +static int af_xdp_umem_create(AFXDPState *s, int sock_fd, Error **err=
p)
> >> +{
> >> +    struct xsk_umem_config config =3D {
...
> >> +    int ret;
> >> +
> >> +    /* Number of descriptors if all 4 queues (rx, tx, cq, fq) are ful=
l. */
> >> +    n_descs =3D (XSK_RING_PROD__DEFAULT_NUM_DESCS
> >> +               + XSK_RING_CONS__DEFAULT_NUM_DESCS) * 2;
> >> +    size =3D n_descs * XSK_UMEM__DEFAULT_FRAME_SIZE;
> >> +
> >> +    s->buffer =3D qemu_memalign(qemu_real_host_page_size(), size);
> >> +    memset(s->buffer, 0, size);
> >> +
> >> +    if (sock_fd < 0) {
> >> +        ret =3D xsk_umem__create(&s->umem, s->buffer, size,
> >> +                               &s->fq, &s->cq, &config);
> >> +    } else {
> >> +#ifdef HAVE_XSK_UMEM__CREATE_WITH_FD
> >> +        ret =3D xsk_umem__create_with_fd(&s->umem, sock_fd, s->buffer=
, size,
> >> +                                       &s->fq, &s->cq, &config);
> >> +#else
> >
> > So sock_fds without HAVE_XSK_UMEM__CREATE_WITH_FD won't work. We'd bett=
er
> >
...
> >> +                         "failed to create umem for %s queue_index: %=
d",
...
> >> +{
> >> +    struct xsk_socket_config cfg =3D {
...
> >> +    };
> >> +    int queue_id, error =3D 0;
> >> +
> >> +    s->inhibit =3D opts->has_inhibit && opts->inhibit;
> >> +    if (s->inhibit) {
> >> +        cfg.libxdp_flags |=3D XSK_LIBXDP_FLAGS__INHIBIT_PROG_LOAD;
> >> +    }
> >> +
...
> >> +    }
> >> +
> >> +    queue_id =3D s->nc.queue_index;
> >> +    if (opts->has_start_queue && opts->start_queue > 0) {
> >> +        queue_id +=3D opts->start_queue;
> >> +    }
> >> +
> >> +    if (opts->has_mode) {
> >> +        /* Specific mode requested. */
> >> +        cfg.xdp_flags |=3D (opts->mode =3D=3D AFXDP_MODE_NATIVE)
...
> >> +        cfg.xdp_flags |=3D XDP_FLAGS_DRV_MODE;
> >> +
> >> +        if (xsk_socket__create(&s->xsk, s->ifname, queue_id,
> >> +                               s->umem, &s->rx, &s->tx, &cfg)) {
...
> >> +                         "failed to create AF_XDP socket for %s queue=
_id: %d",
> >> +                         s->ifname, queue_id);
> >> +        return -1;
> >> +    }
> >> +
> >> +    if (s->inhibit && xsks_map_fd >=3D 0) {
> >> +        int xsk_fd =3D xsk_socket__fd(s->xsk);
> >> +
> >> +        /* Need to update the map manually, libxdp skipped that step.=
 */
> >> +        error =3D bpf_map_update_elem(xsks_map_fd, &queue_id, &xsk_fd=
, 0);
> >> +        if (error) {
> >> +            error_setg_errno(errp, error,
> >> +                             "failed to update xsks map for %s queue_=
id: %d",
> >> +                             s->ifname, queue_id);
> >> +            return -1;
> >> +        }
> >> +    }
> >> +
> >> +    s->xdp_flags =3D cfg.xdp_flags;
> >> +
> >> +    return 0;
> >> +}
> >> +
> >> +/* NetClientInfo methods. */
> >> +static NetClientInfo net_af_xdp_info =3D {
> >> +    .type =3D NET_CLIENT_DRIVER_AF_XDP,
> >> +    .size =3D sizeof(AFXDPState),
> >> +    .receive =3D af_xdp_receive,
> >> +    .poll =3D af_xdp_poll,
> >> +    .cleanup =3D af_xdp_cleanup,
> >> +};
> >> +
> >> +#ifdef HAVE_XSK_UMEM__CREATE_WITH_FD
...
> >> +{
> >> +    gchar **substrings =3D g_strsplit(sock_fds_str, ":", -1);
> >> +    int64_t i, n_sock_fds =3D g_strv_length(substrings);
> >> +    int *sock_fds =3D NULL;
> >> +
> >> +    if (n_sock_fds !=3D n_expected) {
...
> >> +    }
> >> +
> >> +    sock_fds =3D g_new(int, n_sock_fds);
> >> +
> >> +    for (i =3D 0; i < n_sock_fds; i++) {
> >> +        sock_fds[i] =3D monitor_fd_param(monitor_cur(), substrings[i]=
, errp);
> >> +        if (sock_fds[i] < 0) {
> >> +            g_free(sock_fds);
> >> +            sock_fds =3D NULL;
...
> >> +                    const char *name, NetClientState *peer, Error **e=
rrp)
> >> +{
> >> +    const NetdevAFXDPOptions *opts =3D &netdev->u.af_xdp;
> >> +    NetClientState *nc, *nc0 =3D NULL;
> >> +    unsigned int ifindex;
> >> +    uint32_t prog_id =3D 0;
> >> +    int *sock_fds =3D NULL;
> >> +    int xsks_map_fd =3D -1;
> >> +    int64_t i, queues;
> >> +    Error *err =3D NULL;
> >> +    AFXDPState *s;
> >> +
> >> +    ifindex =3D if_nametoindex(opts->ifname);
> >> +    if (!ifindex) {
...
> >> +    }
> >> +
> >> +    queues =3D opts->has_queues ? opts->queues : 1;
> >> +    if (queues < 1) {
...
> >> +    if ((opts->has_inhibit && opts->inhibit) !=3D !!opts->xsks_map_fd=
) {
> >> +        error_setg(errp, "expected 'inhibit=3Don' and 'xsks-map-fd' t=
ogether");
> >> +        return -1;
> >> +    }
> >> +#else
> >> +    if ((opts->has_inhibit && opts->inhibit)
> >> +        !=3D (opts->xsks_map_fd || opts->sock_fds)) {
> >> +        error_setg(errp, "'inhibit=3Don' should be used together with=
 "
...
> >> +        error_setg(errp, "'sock-fds' and 'xsks-map-fd' are mutually e=
xclusive");
> >> +        return -1;
> >> +    }
> >> +
> >> +    if (opts->sock_fds) {
> >> +        sock_fds =3D parse_socket_fds(opts->sock_fds, queues, errp);
> >> +        if (!sock_fds) {
> >> +            return -1;
> >> +        }
> >> +    }
...
> >> +
> >> +    if (opts->xsks_map_fd) {
> >> +        xsks_map_fd =3D monitor_fd_param(monitor_cur(), opts->xsks_ma=
p_fd, errp);
> >> +        if (xsks_map_fd < 0) {
> >> +            goto err;
> >> +        }
...
> >> +        nc =3D qemu_new_net_client(&net_af_xdp_info, peer, "af-xdp", =
name);
> >> +        qemu_set_info_str(nc, "af-xdp%"PRIi64" to %s", i, opts->ifnam=
e);
> >> +        nc->queue_index =3D i;
> >> +
> >> +        if (!nc0) {
...
> >> +        }
> >> +
> >> +        s =3D DO_UPCAST(AFXDPState, nc, nc);
> >> +
> >> +        pstrcpy(s->ifname, sizeof(s->ifname), opts->ifname);
> >> +        s->ifindex =3D ifindex;
> >> +        s->n_queues =3D queues;
> >> +
...
> >> +
> >> +    if (nc0) {
> >> +        s =3D DO_UPCAST(AFXDPState, nc, nc0);
...
> >> --- a/net/clients.h
> >> +++ b/net/clients.h
> >> @@ -64,6 +64,11 @@ int net_init_netmap(const Netdev *netdev, const cha=
r *name,
...
> >>    system_ss.add(when: 'CONFIG_VIRTIO_NET', if_true: files('vhost-user=
.c'), if_false: files('vhost-user-stub.c'))
...
> >> --- a/net/net.c
> >> +++ b/net/net.c
> >> @@ -1082,6 +1082,9 @@ static int (* const net_client_init_fun[NET_CLIE=
NT_DRIVER__MAX])(
> >>  #ifdef CONFIG_NETMAP
> >>          [NET_CLIENT_DRIVER_NETMAP]    =3D net_init_netmap,
> >>  #endif
> >> +#ifdef CONFIG_AF_XDP
> >> +        [NET_CLIENT_DRIVER_AF_XDP]    =3D net_init_af_xdp,
> >> +#endif
...
> >> +#
> >> +# @native: DRV mode, program is attached to a driver, packets are pas=
sed to
...
> >> +#
> >> +# @mode: Attach mode for a default XDP program.  If not specified, th=
en
...
> >> +#     (default: false)
> >> +#
> >> +# @queues: number of queues to be used for multiqueue interfaces (def=
ault: 1).
> >> +#
> >> +# @start-queue: Use @queues starting from this queue number (default:=
 0).
> >> +#
> >> +# @inhibit: Don't load a default XDP program, use one already loaded =
to
> >> +#     the interface (default: false).  Requires @sock-fds or @xsks-ma=
p-fd.
> >> +#
> >> +# @sock-fds: A colon (:) separated list of file descriptors for alrea=
dy open
> >> +#     but not bound AF_XDP sockets in the queue order.  One fd per qu=
eue.
> >> +#     These descriptors should already be added into XDP socket map f=
or
> >> +#     corresponding queues.  Requires @inhibit.
> >> +#
> >> +# @xsks-map-fd: A file descriptor for an already open XDP socket map =
in
...
> >> +    '*start-queue': 'int',
> >> +    '*inhibit':     'bool',
> >> +    '*sock-fds':    { 'type': 'str', 'if': 'HAVE_XSK_UMEM__CREATE_WIT=
H_FD' },
>
...
',
> >>              'dgram', 'vde', 'bridge', 'hubport', 'netmap', 'vhost-use=
r',
> >> -            'vhost-vdpa',
> >> +            'vhost-vdpa', 'af-xdp',
...
> >>      "                VALE port (created on the fly) called 'name' ('n=
mname' is name of the \n"
> >>      "                netmap device, defaults to '/dev/netmap')\n"
> >>  #endif
> >> +#ifdef CONFIG_AF_XDP
> >> +    "-netdev af-xdp,id=3Dstr,ifname=3Dname[,mode=3Dnative|skb][,force=
-copy=3Don|off]\n"
> >> +    "         [,queues=3Dn][,start-queue=3Dm][,inhibit=3Don|off][,xsk=
s-map-fd=3Dk]\n"
> >> +#ifdef HAVE_XSK_UMEM__CREATE_WITH_FD
> >> +    "         [,sock-fds=3Dx:y:...:z]\n"
> >> +#endif
> >> +    "                attach to the existing network interface 'name' =
with AF_XDP socket\n"
> >> +    "                use 'mode=3DMODE' to specify an XDP program atta=
ch mode\n"
> >> +    "                use 'force-copy=3Don|off' to force XDP copy mode=
 even if device supports zero-copy (default: off)\n"
> >> +    "                use 'inhibit=3Don|off' to inhibit loading of a d=
efault XDP program (default: off)\n"
> >> +    "                with inhibit=3Don,\n"
> >> +#ifdef HAVE_XSK_UMEM__CREATE_WITH_FD
> >> +    "                  use 'sock-fds' to provide file descriptors for=
 already open XDP sockets\n"
> >> +    "                  added to a socket map in XDP program.  One soc=
ket per queue.  Or\n"
> >> +#endif
> >> +    "                  use 'xsks-map-fd=3Dk' to provide a file descri=
ptor for xsks map\n"
> >> +    "                use 'queues=3Dn' to specify how many queues of a=
 multiqueue interface should be used\n"
> >> +    "                use 'start-queue=3Dm' to specify the first queue=
 that should be used\n"
> >> +#endif
> >>  #ifdef CONFIG_POSIX
...
> >>      "                (use the -netdev option if possible instead)\n",=
 QEMU_ARCH_ALL)
> >>  SRST
> >> -``-nic [tap|bridge|user|l2tpv3|vde|netmap|vhost-user|socket][,...][,m=
ac=3Dmacaddr][,model=3Dmn]``
> >> +``-nic [tap|bridge|user|l2tpv3|vde|netmap|af-xdp|vhost-user|socket][,=
...][,mac=3Dmacaddr][,model=3Dmn]``
...
> >>
> >> +``-netdev af-xdp,id=3Dstr,ifname=3Dname[,mode=3Dnative|skb][,force-co=
py=3Don|off][,queues=3Dn][,start-queue=3Dm][,inhibit=3Don|off][,xsks-map-fd=
=3Dk][,sock-fds=3Dx:y:...:z]``
> >> +    Configure AF_XDP backend to connect to a network interface 'name'
> >> +    using AF_XDP socket.  A specific program attach mode for a defaul=
t
...
> >> +    defaults to 1.  Traffic arriving on non-configured device queues =
will
...
> >> +            -netdev af-xdp,id=3Dn1,ifname=3Deth0,queues=3D4
> >> +
> >> +    'start-queue' option can be specified if a particular range of qu=
eues
> >> +    [m, m + n] should be in use.  For example, this is necessary in o=
rder
> >> +    to use MLX NICs in native mode.  The driver will create a separat=
e set
> >> +    of queues on top of regular ones, and only these queues can be us=
ed
> >> +    for AF_XDP sockets.  MLX NICs will also require an additional tra=
ffic
...
> start-queue configuration and the extra traffic re-direction.  Unfortunat=
ely,
> this is also not documented anywhere, so you just have to know that it wo=
rks
> this way...
>
> So, one one hand, I understand that it's not the place for vendor-specifi=
c
> docs.  On the other, it gives qemu users a chance to not waste a lot of t=
ime
> trying to figure out why everything is configured correctly, but the traf=
fic
> doesn't flow.
>
...
>     [m, m + n] should be in use.  For example, this is may be necessary i=
n
>     order to use certain NICs in native mode.  Kernel allows the driver t=
o
...
>     may also require an additional traffic redirection with ethtool to th=
ese
>     special queues.
>
>     .. parsed-literal::
...
>         # launch QEMU instance
>         |qemu_system| linux.img -device virtio-net-pci,netdev=3Dn1 \\
>             -netdev af-xdp,id=3Dn1,ifname=3Deth0,queues=3D1,start-queue=
=3D1
>
> What do you think?
...
> >> +        # launch QEMU instance
> >> +        |qemu_system| linux.img -device virtio-net-pci,netdev=3Dn1 \\
> >> +            -netdev af-xdp,id=3Dn1,ifname=3Deth0,queues=3D1,start-que=
ue=3D1
> >> +
> >> +    XDP program can also be loaded externally.  In this case 'inhibit=
' option
> >> +    should be set to 'on' and 'xsks-map-fd' provided with a file desc=
riptor
> >> +    for an open XDP socket map of that program, or 'sock-fds' with fi=
le
> >> +    descriptors for already open but not bound XDP sockets already ad=
ded to a
...
> >> +
> >> +        |qemu_system| linux.img -device virtio-net-pci,netdev=3Dn1 \\
> >> +            -netdev af-xdp,id=3Dn1,ifname=3Deth0,queues=3D3,inhibit=
=3Don,sock-fds=3D15:16:17
> >> +
> >> +    With 'inhibit=3Don' and 'sock-fds', QEMU process will only requir=
e 32 MB
> >> +    of locked memory (RLIMIT_MEMLOCK) per queue or CAP_IPC_LOCK capab=
ility.
> >> +    With 'inhibit=3Don' and 'xsks-map-fd' it will additionally requir=
e
> >> +    CAP_NET_RAW capability.  With 'inhibit=3Doff', CAP_SYS/NET_ADMIN =
should be
> >> +    added as well.
> >
...
`
> >>      Establish a vhost-user netdev, backed by a chardev id. The charde=
v
> >>      should be a unix domain socket backed one. The vhost-user uses a
> >> diff --git a/scripts/ci/org.centos/stream/8/x86_64/configure b/scripts=
/ci/org.centos/stream/8/x86_64/configure
> >> index d02b09a4b9..7585c4c4ed 100755
> >> --- a/scripts/ci/org.centos/stream/8/x86_64/configure
> >> +++ b/scripts/ci/org.centos/stream/8/x86_64/configure
> >> @@ -35,6 +35,7 @@
> >>  --block-drv-ro-whitelist=3D"vmdk,vhdx,vpc,https,ssh" \
...
> >>    printf "%s\n" 'disabled with --disable-FEATURE, default is enabled =
if available'
...
> >>  _meson_option_parse() {
> >>    case $1 in
> >> +    --enable-af-xdp) printf "%s" -Daf_xdp=3Denabled ;;
> >> +    --disable-af-xdp) printf "%s" -Daf_xdp=3Ddisabled ;;
...
er/dockerfiles/debian-amd64.docker
> >> index e39871c7bb..207f7adfb9 100644
> >> --- a/tests/docker/dockerfiles/debian-amd64.docker
> >> +++ b/tests/docker/dockerfiles/debian-amd64.docker
> >> @@ -97,6 +97,7 @@ RUN export DEBIAN_FRONTEND=3Dnoninteractive && \
