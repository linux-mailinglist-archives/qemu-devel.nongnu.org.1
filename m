...
Received: from lists.gnu.org (lists.gnu.org [209.51.188.17])
	by mail.lfdr.de (Postfix) with ESMTPS id 1A1F6AB1BB4
	for <lists+qemu-devel@lfdr.de>; Fri,  9 May 2025 19:41:16 +0200 (CEST)
Received: from localhost ([::1] helo=lists1p.gnu.org)
	by lists.gnu.org with esmtp (Exim 4.90_1)
	(envelope-from <qemu-devel-bounces@nongnu.org>)
	id 1uDRiZ-0000BM-3B; Fri, 09 May 2025 13:41:03 -0400
...
Received: from [185.176.79.56] (helo=frasgout.his.huawei.com)
 ...
 (Exim 4.90_1) (envelope-from <shiju.jose@huawei.com>)
 id 1uDRiG-000070-WF
 for qemu-devel@nongnu.org; Fri, 09 May 2025 13:40:48 -0400
Received: from mail.maildlp.com (unknown [172.18.186.216])
 by frasgout.his.huawei.com (SkyGuard) with ESMTP id 4ZvG191Cvyz6M4YW;
 Sat, 10 May 2025 01:18:21 +0800 (CST)
Received: from frapeml500007.china.huawei.com (unknown [7.182.85.172])
 by mail.maildlp.com (Postfix) with ESMTPS id 753601402EC;
 Sat, 10 May 2025 01:22:55 +0800 (CST)
...
Subject: [PATCH 6/7] hw/cxl: Add Maintenance support
Date: Fri, 9 May 2025 18:22:28 +0100
Message-ID: <20250509172229.726-7-shiju.jose@huawei.com>
...
+        #define PERFORM 0x0
     ...
     CCLS        = 0x41,
@@ -1230,6 +1232,8 @@ typedef struct CXLSupportedFeatureEntry {
...
@@ -1271,6 +1275,28 @@ enum CXL_SET_FEATURE_FLAG_DATA_TRANSFER {
 };
...
@@ -1334,6 +1360,38 @@ static CXLRetCode cmd_features_get_supported(const struct cxl_cmd *cmd,
...
+            break;
         case  CXL_FEATURE_PATROL_SCRUB:
             ...
             get_feats_out->feat_entries[entry++] =
@@ -1432,6 +1490,26 @@ static CXLRetCode cmd_features_get_feature(const struct cxl_cmd *cmd,
         memcpy(payload_out,
...
@@ -1543,6 +1621,42 @@ static CXLRetCode cmd_features_set_feature(const struct cxl_cmd *cmd,
...
@@ -1555,7 +1669,12 @@ static CXLRetCode cmd_features_set_feature(const struct cxl_cmd *cmd,
...
@@ -1565,6 +1684,72 @@ static CXLRetCode cmd_features_set_feature(const struct cxl_cmd *cmd,
...
+
 ...
 static CXLRetCode cmd_identify_memory_device(const struct cxl_cmd *cmd,
                                              uint8_t *payload_in,
@@ -3386,6 +3571,12 @@ static const struct cxl_cmd cxl_cmd_set[256][256] = {
...
index ca61e7c34a..f45296ec3c 100644
--- a/hw/mem/cxl_type3.c
+++ b/hw/mem/cxl_type3.c
@@ -1108,6 +1108,30 @@ void ct3_realize(PCIDevice *pci_dev, Error **errp)
...
+
 ...
 err_release_cdat:
@@ -1733,6 +1757,21 @@ static int ct3d_qmp_cxl_event_log_enc(CxlEventLog log)
         return -EINVAL;
     }
 }
+
...
@@ -1774,6 +1813,11 @@ void qmp_cxl_inject_general_media_event(const char *path, CxlEventLog log,
...
@@ -1817,6 +1861,10 @@ void qmp_cxl_inject_general_media_event(const char *path, CxlEventLog log,
...
@@ -1877,6 +1925,11 @@ void qmp_cxl_inject_dram_event(const char *path, CxlEventLog log, uint8_t flags,
         error_setg(errp, "Unhandled error log type");
         return;
     }
+    if (rc == CXL_EVENT_TYPE_INFO &&
...
@@ -1955,7 +2008,10 @@ void qmp_cxl_inject_dram_event(const char *path, CxlEventLog log, uint8_t flags,
...
index ca515cab13..c73440d90b 100644
--- a/include/hw/cxl/cxl_device.h
+++ b/include/hw/cxl/cxl_device.h
@@ -483,6 +483,12 @@ static inline bool cxl_dev_media_disabled(CXLDeviceState *cxl_dstate)
...
@@ -496,6 +502,13 @@ typedef struct CXLError {
...
+
 typedef struct CXLPoison {
     ...
     uint8_t type;
@@ -508,6 +521,73 @@ typedef struct CXLPoison {
...
@@ -670,6 +750,9 @@ struct CXLType3Dev {
     /* Error injection */
...
@@ -682,6 +765,11 @@ struct CXLType3Dev {
