...
Received: from lists.gnu.org (lists.gnu.org [209.51.188.17])
	by mail.lfdr.de (Postfix) with ESMTPS id 5FD5DAE1033
	for <lists+qemu-devel@lfdr.de>; Fri, 20 Jun 2025 02:12:33 +0200 (CEST)
Received: from localhost ([::1] helo=lists1p.gnu.org)
	by lists.gnu.org with esmtp (Exim 4.90_1)
	(envelope-from <qemu-devel-bounces@nongnu.org>)
	id 1uSPLV-0000gK-F7; Thu, 19 Jun 2025 20:11:05 -0400
Received: from eggs.gnu.org ([2001:470:142:3::10])
 by lists.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1) (envelope-from <stefanha@redhat.com>)
 id 1uSPLS-0000fK-Ac
 for qemu-devel@nongnu.org; Thu, 19 Jun 2025 20:11:02 -0400
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
 s=mimecast20190719; t=1750378258;
 h=from:from:reply-to:subject:subject:date:date:message-id:message-id:
 to:to:cc:cc:mime-version:mime-version:
 content-transfer-encoding:content-transfer-encoding:
 in-reply-to:in-reply-to:references:references;
 bh=UkRFYRAvtQ7gDshao662FTXUMrcW/HWDVfRNnW9WDi4=;
 b=JbNbNqZRDhtxbmREjEnlfVnLqXqe8QAIpbGbARKfn2mqF/NRiZjosypNNLMeqS7/8ZnUnD
 3DjYHcPoDBvXIyWbenp5JCv+NSs/PxR7TwMN5JcNjhgUFv+8k1WgDrfzvdlC+RsbmT+fvy
 0/+iq2HkqtjAuj5+tcSK5ijFqCxqewI=
Received: from mx-prod-mc-05.mail-002.prod.us-west-2.aws.redhat.com
 (ec2-54-186-198-63.us-west-2.compute.amazonaws.com [54.186.198.63]) by
 relay.mimecast.com with ESMTP with STARTTLS (version=TLSv1.3,
 cipher=TLS_AES_256_GCM_SHA384) id us-mta-232-inGI1D1pOFWLcH_bjXPteg-1; Thu,
 19 Jun 2025 20:10:54 -0400
X-MC-Unique: inGI1D1pOFWLcH_bjXPteg-1
X-Mimecast-MFC-AGG-ID: inGI1D1pOFWLcH_bjXPteg_1750378253
Received: from mx-prod-int-01.mail-002.prod.us-west-2.aws.redhat.com
 ...
 id 8243D195609F; Fri, 20 Jun 2025 00:10:53 +0000 (UTC)
Received: from localhost (unknown [10.2.16.120])
 by mx-prod-int-01.mail-002.prod.us-west-2.aws.redhat.com (Postfix) with ESMTP
 id 0701C30001BB; Fri, 20 Jun 2025 00:10:51 +0000 (UTC)
...
Subject: [PATCH v2 04/12] aio-posix: integrate fdmon into glib event loop
Date: Thu, 19 Jun 2025 20:08:20 -0400
Message-ID: <20250620000829.1426291-5-stefanha@redhat.com>
...
index 2e40fff09a..905638bc8d 100644
--- a/util/fdmon-io_uring.c
+++ b/util/fdmon-io_uring.c
@@ -269,6 +269,11 @@ static int process_cq_ring(AioContext *ctx, AioHandlerList *ready_list)
...
+
     io_uring_for_each_cqe(ring, head, cqe) {
         if (process_cqe(ctx, ready_list, cqe)) {
             num_ready++;
@@ -281,6 +286,30 @@ static int process_cq_ring(AioContext *ctx, AioHandlerList *ready_list)
     return num_ready;
 }
...
+
 static int fdmon_io_uring_wait(AioContext *ctx, AioHandlerList *ready_list,
                                int64_t timeout)
 {
@@ -328,12 +357,17 @@ static const FDMonOps fdmon_io_uring_ops = {
...
+
     ...
     if (ret != 0) {
         return false;
@@ -341,6 +375,9 @@ bool fdmon_io_uring_setup(AioContext *ctx)
...
@@ -368,6 +405,11 @@ void fdmon_io_uring_destroy(AioContext *ctx)
