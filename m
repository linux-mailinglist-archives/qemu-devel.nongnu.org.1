...
Received: from lists.gnu.org (lists.gnu.org [209.51.188.17])
	by mail.lfdr.de (Postfix) with ESMTPS id 3114F956FEC
	for <lists+qemu-devel@lfdr.de>; Mon, 19 Aug 2024 18:13:21 +0200 (CEST)
Received: from localhost ([::1] helo=lists1p.gnu.org)
	by lists.gnu.org with esmtp (Exim 4.90_1)
	(envelope-from <qemu-devel-bounces@nongnu.org>)
	id 1sg4zV-00033Z-Hz; Mon, 19 Aug 2024 12:12:21 -0400
Received: from eggs.gnu.org ([2001:470:142:3::10])
 by lists.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1)
 (envelope-from <prvs=5961d01319=ian.brockbank@cirrus.com>)
 id 1sg4zT-0002xP-9V; Mon, 19 Aug 2024 12:12:19 -0400
Received: from mx0b-001ae601.pphosted.com ([67.231.152.168])
 by eggs.gnu.org with esmtps (TLS1.2:ECDHE_RSA_AES_256_GCM_SHA384:256)
 (Exim 4.90_1)
 (envelope-from <prvs=5961d01319=ian.brockbank@cirrus.com>)
 id 1sg4zO-0007Z6-DK; Mon, 19 Aug 2024 12:12:19 -0400
Received: from pps.filterd (m0077474.ppops.net [127.0.0.1])
 by mx0b-001ae601.pphosted.com (8.18.1.2/8.18.1.2) with ESMTP id 47JF6kmr012798;
 Mon, 19 Aug 2024 11:11:57 -0500
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=cirrus.com; h=cc
 :content-transfer-encoding:content-type:date:from:in-reply-to
 :message-id:mime-version:references:subject:to; s=
 PODMain02222019; bh=j7hGApsJVqvt0vzVQMiwFY612vDcBZvRRAiTdsp0Aq0=; b=
 
Received: from ausex02.ad.cirrus.com ([141.131.3.21])
 by mx0b-001ae601.pphosted.com (PPS) with ESMTPS id 412r9hsxbq-1
 (version=TLSv1.2 cipher=ECDHE-RSA-AES256-GCM-SHA384 bits=256 verify=NOT);
 Mon, 19 Aug 2024 11:11:56 -0500 (CDT)
...
Received: from EDIN7BQBTG3.ad.cirrus.com (141.131.38.212) by
 ...
 15.2.1544.9 via Frontend Transport; Mon, 19 Aug 2024 11:11:48 -0500
...
CC: Palmer Dabbelt <palmer@dabbelt.com>, Alistair Francis
 ...
 <ian.brockbank@cirrus.com>, Ian Brockbank <Ian.Brockbank@cirrus.com>, Troy
 Song <wb-szh830103@alibaba-inc.com>
Subject: [PATCH 11/11 v2] tests: add riscv clic qtest case and a function in
 qtest
Date: Mon, 19 Aug 2024 17:02:22 +0100
Message-ID: <20240819160742.27586-15-Ian.Brockbank@cirrus.com>
...
X-Proofpoint-GUID: wHjusZ7qPR85PS3mZhD8ZOwGepVPfC6d
X-Proofpoint-ORIG-GUID: wHjusZ7qPR85PS3mZhD8ZOwGepVPfC6d
...
In order to detect the interrupts, qtest.c has been updated to send interru=
pt
information back to the test about the IRQ being delivered. Since we need t=
o
...
@@ -34,6 +34,8 @@ void qtest_server_init(const char *qtest_chrdev, const ch=
ar *qtest_log, Error **
...
+/*
+ * Encoding for passing the specific IRQ information from an interrupt han=
dler
+ * to QTest. This needs to support CLIC, which has a 12-bit interrupt numb=
er.
+ */
...
@@ -369,6 +400,7 @@ static void qtest_process_command(CharBackend *chr, gch=
ar **words)
         bool is_named;
         bool is_outbound;
...
@@ -386,38 +418,46 @@ static void qtest_process_command(CharBackend *chr, g=
char **words)
             return;
         }
...
@@ -499,6 +500,7 @@ static QTestState *qtest_init_internal(const char *qemu=
_bin,
...
+}
+
 void qtest_module_load(QTestState *s, const char *prefix, const char *libn=
ame)
 {
...
@@ -366,6 +366,15 @@ void qtest_module_load(QTestState *s, const char *pref=
ix, const char *libname);
  */
...
@@ -258,7 +258,8 @@ qtests_s390x =3D \
 ...
 qtests_riscv32 =3D \
-  (config_all_devices.has_key('CONFIG_SIFIVE_E_AON') ? ['sifive-e-aon-watc=
hdog-test'] : [])
+  (config_all_devices.has_key('CONFIG_SIFIVE_E_AON') ? ['sifive-e-aon-watc=
hdog-test'] : []) + \
...
diff --git a/tests/qtest/riscv32-clic-test.c b/tests/qtest/riscv32-clic-tes=
t.c
...
+ * by appending the string with them. E.g. QEMU_BASE_ARGS ",clic-mode=3Dm"=
.
...
+#define NMBITS_MASK_2   0x00000010      /* PRV_M plus either PRV_S or PRV_=
U */
...
+#define TEST_CFG_SU(_nmb)   (TEST_CFG(_nmb) & SUCFG_MASK)   /* PRV_S in MS=
U */
+#define TEST_CFG_MU(_nmb)   (TEST_CFG(_nmb) & MUCFG_MASK)   /* PRV_M in MU=
 */
+#define TEST_CFG_MS(_nmb)   (TEST_CFG(_nmb) & MSCFG_MASK)   /* PRV_M in MS=
 */
+#define TEST_CFG_MSU(_nmb)  (TEST_CFG(_nmb) & MSUCFG_MASK)  /* PRV_M in MS=
U */
+
+/*
...
+ */
+GEN_CHECK_REG_MMIO_L(cliccfg_supported_max_mnlbits, MCLICCFG_ADDR, 0x8, 0x=
8)
+
+/*
...
+ * Requires PRV_S mode
+ */
+GEN_CHECK_REG_MMIO_L(cliccfg_min_snlbits_s, MCLICCFG_ADDR, 0x00004, 0x0000=
4)
+
+/*
...
+ * Requires PRV_S mode
+ */
+GEN_CHECK_REG_MMIO_L(cliccfg_min_unlbits_u, MCLICCFG_ADDR, 0x0000004, 0x00=
00004)
+
+/*
...
+ * set nmbits =3D 0, mnlbits =3D 4, snlbits =3D 0, unlbits =3D 8
+ */
+GEN_CHECK_REG_MMIO_L(cliccfg_unlbits_no_u, MCLICCFG_ADDR, 0x8000004, 0x000=
0004)
+
+/*
+ * Set all modes
...
+GEN_CHECK_REG_MMIO_L(cliccfg_unsupported_nmbits_1, MCLICCFG_ADDR, 0x18, 0x=
08)
+
+/*
...
+GEN_CHECK_REG_MMIO_L(cliccfg_unsupported_nmbits_2, MCLICCFG_ADDR, 0x28, 0x=
08)
+
+/*
...
+ */
+GEN_CHECK_REG_MMIO_L(cliccfg_unsupported_nmbits_3, MCLICCFG_ADDR, 0x38, 0x=
08)
+
+/*
+ * clicintie tests
...
+ *
+ * Layout:
+ *  [7:6]   mode        b00 =3D U, b01 =3D S, b10 =3D reserved, b11 =3D M
+ *  [5:3]   reserved
+ *  [2:1]   trig        trig[0]: 0 =3D level, 1 =3D edge; trig[1]: 0 =3D p=
os, 1 =3D neg
+ *  [0]     shv         0 =3D non-vectored, 1 =3D vectored
+ */
+
+/* Mode tests - note these deliberately use different trig and int setting=
s */
+/*
+ * Set mode 3 - PRV_M
...
+ * clicintattr[i] =3D 0xc1
+ */
+GEN_CHECK_REG_MMIO_B(clicintattr_positive_edge_triggered, clicintattr12_ad=
dr,
+                     0xc3, 0xc3)
+
+/*
...
+ * clicintattr[i] =3D 0xc7
+ */
+GEN_CHECK_REG_MMIO_B(clicintattr_negative_edge_triggered, clicintattr12_ad=
dr,
+                     0xc7, 0xc7)
+
+/*
...
+ * clicintattr[i] =3D 0xc1
+ */
+GEN_CHECK_REG_MMIO_B(clicintattr_positive_level_triggered, clicintattr12_a=
ddr,
+                     0xc1, 0xc1)
+
+/*
...
+ * clicintattr[i] =3D 0xc5
+ */
+GEN_CHECK_REG_MMIO_B(clicintattr_negative_level_triggered, clicintattr12_a=
ddr,
+                     0xc5, 0xc5)
+
+/*
...
+ *
+ * IRQs configured as PRV_M in the mode field of intattr are not visible v=
ia
...
+    int default_intattr =3D INTATTR_SHV | (TRIG_LEVEL << INTATTR_TRIG_SHIF=
T);
+    int default_reg_value =3D (default_intattr << INTATTR_SHIFT) |
+                            (PRV_M << REG_MODE_SHIFT) | (0x7 << INTCTL_SHI=
FT);
...
+    g_assert_cmpuint(qtest_readl(qts, MCLICCFG_ADDR), =3D=3D , TEST_CFG_MS=
(1));
+
+    qtest_writel(qts, clicint12_addr, default_reg_value);
+    g_assert_cmpuint(qtest_readl(qts, clicint12_addr), =3D=3D , default_re=
g_value);
+    qtest_writel(qts, clicint25_addr, default_reg_value);
+    g_assert_cmpuint(qtest_readl(qts, clicint25_addr), =3D=3D , default_re=
g_value);
+    qtest_writel(qts, clicint26_addr, default_reg_value);
+    g_assert_cmpuint(qtest_readl(qts, clicint26_addr), =3D=3D , default_re=
g_value);
+
+    /*
...
+    g_assert_cmpuint(qtest_readl(qts, SCLICCFG_ADDR), =3D=3D , TEST_CFG_S(=
1));
+
+    /* These are all PRV_M mode so reading via PRV_S should see them all a=
s 0 */
...
+     */
+    value =3D (default_reg_value & ~REG_MODE_MASK) | (PRV_S << REG_MODE_SH=
IFT);
...
+    g_assert_cmpuint(qtest_readl(qts, clicint12_addr), =3D=3D , reg_value_=
2);
+    g_assert_cmpuint(qtest_readl(qts, clicint12_addr_s), =3D=3D , reg_valu=
e_2);
+
+    /* ...but not the others */
+    qtest_writel(qts, clicint25_addr_s, 0x55555555);
...
+    g_assert_cmpuint(qtest_readl(qts, MCLICCFG_ADDR), =3D=3D , TEST_CFG_MU=
(1));
+
+    qtest_writel(qts, clicint12_addr, default_reg_value);
+    g_assert_cmpuint(qtest_readl(qts, clicint12_addr), =3D=3D , default_re=
...
+
+    /*
+     * Now check the PRV_U view. Note we only have one additional mode, so=
 we
+     * use the xxx_addr_s register bank.
+     */
+
+     /* We should only see the PRV_U part of CLICCFG */
+    g_assert_cmpuint(qtest_readl(qts, SCLICCFG_ADDR), =3D=3D , TEST_CFG_U(=
1));
+
+    /* These are all PRV_M mode so reading via PRV_S should see them all a=
s 0 */
...
+     */
+    value =3D (default_reg_value & ~REG_MODE_MASK) | (PRV_U << REG_MODE_SH=
IFT);
+    qtest_writel(qts, clicint12_addr, value);
+    g_assert_cmpuint(qtest_readl(qts, clicint12_addr_s), =3D=3D , value);
+
...
+ * IRQs configured as PRV_M in the mode field of intattr are not visible v=
ia
+ * the PRV_S or PRV_U registers, and all registers appear as hard-wired ze=
ros.
+ */
...
+    g_assert_cmpuint(qtest_readl(qts, SCLICCFG_ADDR), =3D=3D , TEST_CFG_SU=
(2));
+
+    /* These are all PRV_M mode so reading via PRV_S should see them all a=
s 0 */
...
+
+     /* We should only see the PRV_U part of CLICCFG */
+    g_assert_cmpuint(qtest_readl(qts, UCLICCFG_ADDR), =3D=3D , TEST_CFG_U(=
2));
+
+    /* These are all PRV_M or PRV_S so reading via PRV_U should see them a=
s 0 */
...
+    g_assert_cmpuint(qtest_readl(qts, clicint25_addr), =3D=3D , reg_value_=
3);
+    g_assert_cmpuint(qtest_readl(qts, clicint25_addr_s), =3D=3D , reg_valu=
e_3);
+    g_assert_cmpuint(qtest_readl(qts, clicint25_addr_u), =3D=3D , reg_valu=
e_3);
...
+
+    /* clicintattr mode configure cases are all PRV_M WARL - nmbits =3D=3D=
 0*/
...
,
+                   test_configure_clicintattr_unsupported_mode_10);
+    qtest_add_func("virt/clic/prv_ms/intattr_prv_u_to_s_warl_nmbits_1_prv_=
m",
...
,
+                   test_configure_clicintattr_unsupported_mode_10);
+    qtest_add_func("virt/clic/prv_mu/intattr_prv_s_to_u_warl_nmbits_1_prv_=
m",
...
+                   test_configure_clicintattr_prv_u_supported);
+
+    /* clicintattr mode configure cases with nmbits =3D 1 =3D> PRV_M and P=
RV_S */
...
",
+                   test_configure_clicintattr_unsupported_mode_10);
+    qtest_add_func("virt/clic/prv_msu/intattr_prv_u_to_s_warl_nmbits_1_prv=
_m",
+                   test_configure_clicintattr_prv_u_to_s_warl);
...
+{                                                                         =
  \
+    g_autofree char *prefix =3D g_strdup_printf("virt/clic/clicintl_%d_bit=
s", \
+                                              _nbits);                    =
  \
...
+    qtest_add_func("virt/clic/unvectored_positive_level_triggered_interrup=
t",
+                   test_unvectored_positive_level_triggered_interrupt);
+    qtest_add_func("virt/clic/unvectored_negative_level_triggered_interrup=
t",
