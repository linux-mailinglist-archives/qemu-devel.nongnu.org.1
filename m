...
Received: by mail-pg1-x52f.google.com with SMTP id
 41be03b00d2f7-7cd830e0711so3669888a12.0
 for <qemu-devel@nongnu.org>; Mon, 26 Aug 2024 19:18:40 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 d=rivosinc-com.20230601.gappssmtp.com; s=20230601; t=1724725119; x=1725329919;
 darn=nongnu.org; 
 h=content-transfer-encoding:cc:to:subject:message-id:date:from
 :in-reply-to:references:mime-version:from:to:cc:subject:date
 :message-id:reply-to;
 bh=fnckI1QIsQafhTdEDSOTRn7zEKf3xulYG//5Lop74c0=;
 b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
 d=1e100.net; s=20230601; t=1724725119; x=1725329919;
 h=content-transfer-encoding:cc:to:subject:message-id:date:from
 :in-reply-to:references:mime-version:x-gm-message-state:from:to:cc
 :subject:date:message-id:reply-to;
 bh=fnckI1QIsQafhTdEDSOTRn7zEKf3xulYG//5Lop74c0=;
 b=
X-Forwarded-Encrypted: i=1;
 AJvYcCU2rQKS3ZOskkmRWPbAe9GmRCB9ia06fbbojEjmlz0BVHFte8lFiublg2BvM6mvzrJXIHkAYmVoxUS9@nongnu.org
X-Gm-Message-State: AOJu0YyCiR/8MCRVNzVVG47ohpgOuIkEQFZ01UpMb0+pD7O/bEaHrUTj
 W2hSLuq3PQ0QN1JSlDy/wK5O6/XjPSE4cl+yWlwCnBqQuDzzNUyhaCfpXIni66wqo4t0Ui/nOue
 vQXnhrLKXW5x4SKriPk0rUankvlWpYoQITjTNcw==
X-Google-Smtp-Source: AGHT+IEkAwTUHLkYTnY+iTiMTSHKCAAZfGFJP1tLVmk/XVnOw3uTbgPvutwREhlylnYaKAFE3zQx3X7ifWN9bO0z9rc=
X-Received: by 2002:a17:90a:684f:b0:2cb:5829:a491 with SMTP id
 98e67ed59e1d1-2d824d08e6cmr2228116a91.20.1724725118335; Mon, 26 Aug 2024
 19:18:38 -0700 (PDT)
MIME-Version: 1.0
...
In-Reply-To: <d1fdc3f6-fa9d-4aa4-9899-06105f88f68d@ventanamicro.com>
From: Tomasz Jeznach <tjeznach@rivosinc.com>
Date: Mon, 26 Aug 2024 19:18:27 -0700
Message-ID: <CAH2o1u7NQ+zqVSSj+xXWiCKGXbkdivj_u8XW6mchucizzmzsxg@mail.gmail.com>
...
scv-iommu.pdf
> >>
> >> Add the foundation of the device emulation for RISC-V IOMMU. It includ=
es
> >> support for s-stage (sv32, sv39, sv48, sv57 caps) and g-stage (sv32x4,
> >> sv39x4, sv48x4, sv57x4 caps).
> >>
> >> Other capabilities like ATS and DBG support will be added incrementall=
y
> >> in the next patches.
> >>
...
> >>   riscv_ss.add(when: 'CONFIG_MICROCHIP_PFSOC', if_true: files('microch=
ip_pfsoc.c'))
> >>   riscv_ss.add(when: 'CONFIG_ACPI', if_true: files('virt-acpi-build.c'=
))
> >> +riscv_ss.add(when: 'CONFIG_RISCV_IOMMU', if_true: files('riscv-iommu.=
c'))
...
> > I think the IOVA here may be a GVA and we should not use a GVA to perfo=
rm msi check. Section 2.3 states that we should always walk first stage pag=
e table to get the GPA (step 17) and then use the GPA to do the msi check (=
step 18).
>
> That's a good point. Even if we rule out the address as a GVA by doing a =
second stage
> check (via en_g) we're still assuming that iotlb->iova =3D GPA without go=
ing through
> the first stage translation. We're basically gambling for a match calling
> riscv_iommu_msi_check() this early.
>
> I'll remove this early check entirely since we're doing a proper msi chec=
k with a
...
I'd suggest fixing the missing en_s check, to enable proper handling of MSI=
P.
...
>
>
> This will be suboptimal for cases where IOVA =3D=3D GPA and we would have=
 an early exit by
> guessing it right, but I'm more confortable adhering fully with the spec =
this time.
>
>
> Thanks,
...
> >> +                sv_mode =3D pass ? RISCV_IOMMU_CAP_SV32X4 : RISCV_IOM=
MU_CAP_SV32;
> >> +                if (!(s->cap & sv_mode)) {
...
> >> +                sv_mode =3D pass ? RISCV_IOMMU_CAP_SV39X4 : RISCV_IOM=
MU_CAP_SV39;
> >> +                if (!(s->cap & sv_mode)) {
> >> +                    return RISCV_IOMMU_FQ_CAUSE_DDT_MISCONFIGURED;
> >> +                }
...
> >> +                sv_mode =3D pass ? RISCV_IOMMU_CAP_SV48X4 : RISCV_IOM=
MU_CAP_SV48;
> >> +                if (!(s->cap & sv_mode)) {
> >> +                    return RISCV_IOMMU_FQ_CAUSE_DDT_MISCONFIGURED;
> >> +                }
...
> >> +                sv_mode =3D pass ? RISCV_IOMMU_CAP_SV57X4 : RISCV_IOM=
MU_CAP_SV57;
> >> +                if (!(s->cap & sv_mode)) {
> >> +                    return RISCV_IOMMU_FQ_CAUSE_DDT_MISCONFIGURED;
> >> +                }
...
> >> +        const unsigned va_skip =3D TARGET_PAGE_BITS + sc[pass].ptidxb=
its *
...
> >> +        const bool ade =3D
> >> +            ctx->tc & (pass ? RISCV_IOMMU_DC_TC_GADE : RISCV_IOMMU_DC=
_TC_SADE);
> >> +
...
> >> +                sc[pass].ptesize, MEMTXATTRS_UNSPECIFIED) !=3D MEMTX_=
OK) {
> >> +            return (iotlb->perm & IOMMU_WO) ? RISCV_IOMMU_FQ_CAUSE_WR=
_FAULT
> >> +                                            : RISCV_IOMMU_FQ_CAUSE_RD=
_FAULT;
> >> +        }
> >> +
...
/
> >> +        } else if ((pte & (PTE_R | PTE_W | PTE_X)) =3D=3D (PTE_W | PT=
E_X)) {
> >> +            break;                /* Reserved leaf PTE flags: PTE_W +=
 PTE_X */
...
;
> >> +            /* Update address mask based on smallest translation gran=
ularity */
...
> >> +                                     uint32_t fault_type, uint32_t ca=
use,
> >> +                                     bool pv,
> >> +                                     uint64_t iotval, uint64_t iotval=
2)
> >> +{
...
> >> +
> >> +    if (pv) {
> >> +        ev.hdr =3D set_field(ev.hdr, RISCV_IOMMU_FQ_HDR_PID, ctx->pro=
cess_id);
> >> +    }
> >> +
> >> +    ev.iotval =3D iotval;
...
> >> +
> >> +    /* fetch MSI PTE */
> >> +    addr =3D PPN_PHYS(get_field(ctx->msiptp, RISCV_IOMMU_DC_MSIPTP_PP=
N));
...
> >> +    le64_to_cpus(&pte[1]);
> >> +
> >> +    if (!(pte[0] & RISCV_IOMMU_MSI_PTE_V) || (pte[0] & RISCV_IOMMU_MS=
I_PTE_C)) {
> >> +        /*
> >> +         * The spec mentions that: "If msipte.C =3D=3D 1, then furthe=
r
...
> >> +
> >> +        trace_riscv_iommu_msi(s->parent_obj.id, PCI_BUS_NUM(ctx->devi=
d),
> >> +                              PCI_SLOT(ctx->devid), PCI_FUNC(ctx->dev=
id),
> >> +                              gpa, addr);
> >> +
> >> +        res =3D dma_memory_write(s->target_as, addr, &data, size, att=
rs);
> >> +        if (res !=3D MEMTX_OK) {
...
> >> +     * Report an error for interrupt identities exceeding the maximum=
 allowed
> >> +     * for an IMSIC interrupt file (2047) or destination address is n=
ot 32-bit
...
> >> +        msi_mode =3D get_field(ctx->msiptp, RISCV_IOMMU_DC_MSIPTP_MOD=
E);
> >> +
...
> >> + *
> >> + * @s         : IOMMU Device State
> >> + * @ctx       : Device Translation Context with devid and process_id =
set.
> >> + * @return    : success or fault code.
> >> + */
> >> +static int riscv_iommu_ctx_fetch(RISCVIOMMUState *s, RISCVIOMMUContex=
t *ctx)
> >> +{
...
> >> +    /* Device Context format: 0: extended (64 bytes) | 1: base (32 by=
tes) */
...
> >> +     */
> >> +    if (ctx->devid >=3D (1 << (depth * 9 + 6 + (dc_fmt && depth !=3D =
2)))) {
...
> >> +    for (depth =3D mode - RISCV_IOMMU_DC_FSC_PDTP_MODE_PD8; depth-- >=
 0; ) {
> >> +        /*
> >> +         * Select process id index bits based on process directory tr=
ee
> >> +         * level. See IOMMU Specification, 2.2. Process-Directory-Tab=
le.
> >> +         */
...
> >> +}
> >> +
> >> +static void __ctx_inval_all(gpointer key, gpointer value, gpointer da=
ta)
> >> +{
> >> +    RISCVIOMMUContext *ctx =3D (RISCVIOMMUContext *) value;
> >> +    if (ctx->tc & RISCV_IOMMU_DC_TC_V) {
...
> >> +}
> >> +
> >> +/* Find or allocate translation context for a given {device_id, proce=
ss_id} */
> >> +static RISCVIOMMUContext *riscv_iommu_ctx(RISCVIOMMUState *s,
> >> +                                          unsigned devid, unsigned pr=
ocess_id,
> >> +                                          void **ref)
> >> +{
> >> +    GHashTable *ctx_cache;
...
> >> +            g_hash_table_unref(ctx_cache);
> >> +            ctx_cache =3D g_hash_table_new_full(__ctx_hash, __ctx_equ=
al,
> >> +                                              g_free, NULL);
...
> >> +
> >> +    /*
> >> +     * TC[32] is reserved for custom extensions, used here to tempora=
rily
> >> +     * enable automatic page-request generation for ATS queries.
> >> +     */
> >> +    enable_pri =3D (iotlb->perm =3D=3D IOMMU_NONE) && (ctx->tc & BIT_=
ULL(32));
...
> >> +                               RISCV_IOMMU_PREQ_HDR_PID, ctx->process=
_id);
> >> +        }
> >> +        pr.hdr =3D set_field(pr.hdr, RISCV_IOMMU_PREQ_HDR_DID, ctx->d=
evid);
...
> >> +static MemTxResult riscv_iommu_iofence(RISCVIOMMUState *s, bool notif=
y,
> >> +    uint64_t addr, uint32_t data)
> >> +{
> >> +    /*
> >> +     * ATS processing in this implementation of the IOMMU is synchron=
ous,
...
> >> +    uint64_t new_ddtp =3D riscv_iommu_reg_get64(s, RISCV_IOMMU_REG_DD=
TP);
...
> >> +
> >> +    if (ok) {
> >> +        /* clear reserved and busy bits, report back sanitized versio=
n */
...
> >> +    tail =3D riscv_iommu_reg_get32(s, RISCV_IOMMU_REG_CQT) & s->cq_ma=
sk;
> >> +    head =3D riscv_iommu_reg_get32(s, RISCV_IOMMU_REG_CQH) & s->cq_ma=
sk;
> >> +
...
> >> +        }
> >> +
> >> +        trace_riscv_iommu_cmd(s->parent_obj.id, cmd.dword0, cmd.dword=
1);
> >> +
> >> +        cmd_opcode =3D get_field(cmd.dword0,
> >> +                               RISCV_IOMMU_CMD_OPCODE | RISCV_IOMMU_C=
MD_FUNC);
> >> +
> >> +        switch (cmd_opcode) {
...
> >> +{
> >> +    uint64_t base;
> >> +    uint32_t ctrl_set =3D riscv_iommu_reg_get32(s, RISCV_IOMMU_REG_CQ=
CSR);
> >> +    uint32_t ctrl_clr;
...
> >> +        s->cq_mask =3D (2ULL << get_field(base, RISCV_IOMMU_CQB_LOG2S=
Z)) - 1;
...
|
> >> +                   RISCV_IOMMU_CQCSR_CMD_ILL | RISCV_IOMMU_CQCSR_CMD_=
TO |
...
> >> +    }
> >> +
> >> +    riscv_iommu_reg_mod32(s, RISCV_IOMMU_REG_CQCSR, ctrl_set, ctrl_cl=
r);
> >> +}
> >> +
...
> >> +{
> >> +    uint64_t base;
> >> +    uint32_t ctrl_set =3D riscv_iommu_reg_get32(s, RISCV_IOMMU_REG_FQ=
CSR);
> >> +    uint32_t ctrl_clr;
...
> >> +        s->fq_mask =3D (2ULL << get_field(base, RISCV_IOMMU_FQB_LOG2S=
Z)) - 1;
...
> >> +    }
> >> +
> >> +    riscv_iommu_reg_mod32(s, RISCV_IOMMU_REG_FQCSR, ctrl_set, ctrl_cl=
r);
> >> +}
> >> +
...
> >> +{
> >> +    uint64_t base;
> >> +    uint32_t ctrl_set =3D riscv_iommu_reg_get32(s, RISCV_IOMMU_REG_PQ=
CSR);
> >> +    uint32_t ctrl_clr;
...
> >> +        s->pq_mask =3D (2ULL << get_field(base, RISCV_IOMMU_PQB_LOG2S=
Z)) - 1;
...
> >> +    }
> >> +
> >> +    riscv_iommu_reg_mod32(s, RISCV_IOMMU_REG_PQCSR, ctrl_set, ctrl_cl=
r);
> >> +}
> >> +
...
> >> +
> >> +    s->icvec_update(s,
> >> +                    riscv_iommu_get_icvec_vector(icvec, RISCV_IOMMU_I=
NTR_CQ),
> >> +                    riscv_iommu_get_icvec_vector(icvec, RISCV_IOMMU_I=
NTR_FQ),
> >> +                    riscv_iommu_get_icvec_vector(icvec, RISCV_IOMMU_I=
...
> >> +        stw_le_p(&s->regs_rw[addr], ((rw & ro) | (data & ~ro)) & ~(da=
ta & wc));
> >> +    } else if (size =3D=3D 4) {
> >> +        uint32_t ro =3D ldl_le_p(&s->regs_ro[addr]);
> >> +        uint32_t wc =3D ldl_le_p(&s->regs_wc[addr]);
> >> +        uint32_t rw =3D ldl_le_p(&s->regs_rw[addr]);
> >> +        stl_le_p(&s->regs_rw[addr], ((rw & ro) | (data & ~ro)) & ~(da=
ta & wc));
> >> +    } else if (size =3D=3D 8) {
> >> +        uint64_t ro =3D ldq_le_p(&s->regs_ro[addr]);
> >> +        uint64_t wc =3D ldq_le_p(&s->regs_wc[addr]);
> >> +        uint64_t rw =3D ldq_le_p(&s->regs_rw[addr]);
> >> +        stq_le_p(&s->regs_rw[addr], ((rw & ro) | (data & ~ro)) & ~(da=
ta & wc));
> >> +    }
> >> +
...
> >> +
> >> +/*
> >> + * Translations matching MSI pattern check are redirected to "riscv-i=
ommu-trap"
> >> + * memory region as untranslated address, for additional MSI/MRIF int=
erception
> >> + * by IOMMU interrupt remapping implementation.
> >> + * Note: Device emulation code generating an MSI is expected to provi=
de a valid
...
> >> +    if (s->enable_msi) {
> >> +        s->cap |=3D RISCV_IOMMU_CAP_MSI_FLAT | RISCV_IOMMU_CAP_MSI_MR=
IF;
> >> +    }
> >> +    if (s->enable_s_stage) {
...
> >> +
> >> +    /* Out-of-reset translation mode: OFF (DMA disabled) BARE (passth=
rough) */
> >> +    s->ddtp =3D set_field(0, RISCV_IOMMU_DDTP_MODE, s->enable_off ?
> >> +                        RISCV_IOMMU_DDTP_MODE_OFF : RISCV_IOMMU_DDTP_=
MODE_BARE);
> >> +
> >> +    /* register storage */
...
> >> +     */
> >> +    memory_region_init_io(&s->regs_mr, OBJECT(dev), &riscv_iommu_mmio=
_ops, s,
...
> >> +    stl_le_p(&s->regs_wc[RISCV_IOMMU_REG_CQCSR], RISCV_IOMMU_CQCSR_CQ=
MF |
> >> +        RISCV_IOMMU_CQCSR_CMD_TO | RISCV_IOMMU_CQCSR_CMD_ILL);
> >> +    stl_le_p(&s->regs_ro[RISCV_IOMMU_REG_CQCSR], RISCV_IOMMU_CQCSR_CQ=
ON |
> >> +        RISCV_IOMMU_CQCSR_BUSY);
> >> +    stl_le_p(&s->regs_wc[RISCV_IOMMU_REG_FQCSR], RISCV_IOMMU_FQCSR_FQ=
MF |
> >> +        RISCV_IOMMU_FQCSR_FQOF);
> >> +    stl_le_p(&s->regs_ro[RISCV_IOMMU_REG_FQCSR], RISCV_IOMMU_FQCSR_FQ=
ON |
> >> +        RISCV_IOMMU_FQCSR_BUSY);
> >> +    stl_le_p(&s->regs_wc[RISCV_IOMMU_REG_PQCSR], RISCV_IOMMU_PQCSR_PQ=
MF |
> >> +        RISCV_IOMMU_PQCSR_PQOF);
> >> +    stl_le_p(&s->regs_ro[RISCV_IOMMU_REG_PQCSR], RISCV_IOMMU_PQCSR_PQ=
ON |
...
> >> +    trace_riscv_iommu_dma(as->iommu->parent_obj.id, PCI_BUS_NUM(as->d=
evid),
> >> +                          PCI_SLOT(as->devid), PCI_FUNC(as->devid), i=
ommu_idx,
...
> >> +{
> >> +    return pci_get_word(pdev->config + PCI_CLASS_DEVICE) =3D=3D 0x080=
6;
> >> +}
> >> +
...
int64_t iova, uint64_t phys) "%s: translate %04x:%02x.%u MSI 0x%"PRIx64" ->=
 0x%"PRIx64
> >> +riscv_iommu_mrif_notification(const char *id, uint32_t nid, uint64_t =
phys) "%s: sent MRIF notification 0x%x to 0x%"PRIx64
